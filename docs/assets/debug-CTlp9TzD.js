import{Ao as e,As as t,Co as n,Cs as r,Ct as i,Ds as a,Es as o,Ms as s,N as c,Os as l,P as u,Ss as d,To as f,Ts as p,Tt as m,_n as h,_o as g,_s as _,bo as v,gn as y,go as b,ha as x,ho as S,hs as C,js as w,ks as T,ma as E,mo as D,ms as ee,pa as te,po as ne,ro as re,so as ie,vo as ae,vs as O,wo as k,ws as oe,xo as se,xs as ce,ys as le}from"./wordmark-DBxhtYW2.js";var A=class extends Error{constructor(e){super(e),this.name=`GpuCompileError`}};function j(e,t){let n=Math.fround(e);if(!Number.isFinite(n))throw new A(`${t}: value ${e} is not representable as a finite f32 (WGSL kernels compute in f32; keep magnitudes within ~3.4e38)`);return Object.is(n,-0)?`-0f`:`${String(n)}f`}function ue(e){return`${e>>>0}u`}function M(e){return`0x${(e>>>0).toString(16).padStart(8,`0`)}u`}var N=M,de=j(34028234663852886e22,`internal f32 max`);function fe(e,t){let n=N(e);for(let e of t)n=`pcg_hash_mix(${n}, ${e})`;return`pcg_hash_finalize(${n})`}function pe(){let e=[];for(let t=0;t<12;t++){let r=e=>j(n[t*3+e],`internal GRAD3`);e.push(`  vec3<f32>(${r(0)}, ${r(1)}, ${r(2)}),`)}return`var<private> PCG_GRAD3: array<vec3<f32>, 12> = array<vec3<f32>, 12>(
${e.join(`
`)}
  ...
  return f32(h >> 8u) * ${j(p,`internal hashFloat scale`)};
  ...
  return ${P(`internal PERLIN_SCALE`)(se)} * pcg_mix(
  ...
  let t = f32(i + j + k) * ${P(`internal simplex G3`)(b)};
  let x0 = p.x - (f32(i) - t);
  let y0 = p.y - (f32(j) - t);
  let z0 = p.z - (f32(k) - t);
  var i1: i32;
  ...
  let x1 = x0 - f32(i1) + ${P(`internal simplex G3`)(b)};
  let y1 = y0 - f32(j1) + ${P(`internal simplex G3`)(b)};
  let z1 = z0 - f32(k1) + ${P(`internal simplex G3`)(b)};
  let x2 = x0 - f32(i2) + ${P(`internal simplex 2*G3`)(2*b)};
  let y2 = y0 - f32(j2) + ${P(`internal simplex 2*G3`)(2*b)};
  let z2 = z0 - f32(k2) + ${P(`internal simplex 2*G3`)(2*b)};
  let x3 = x0 - 1f + ${P(`internal simplex 3*G3`)(3*b)};
  let y3 = y0 - 1f + ${P(`internal simplex 3*G3`)(3*b)};
  let z3 = z0 - 1f + ${P(`internal simplex 3*G3`)(3*b)};
