import{$n as e,$r as t,Hn as n,Kn as r,Qn as i,Qr as a,Vt as o,ai as s,ar as c,ci as l,cr as u,di as d,er as f,fi as p,gi as m,hi as h,hr as g,li as _,lr as v,mi as y,ni as b,nr as x,oi as S,or as C,pi as w,ri as T,rr as E,si as D,ti as O,tr as k,ui as ee,ur as te}from"./wordmark-CgtiJbEi.js";var A=class extends Error{constructor(e){super(e),this.name=`GpuCompileError`}};function j(e,t){let n=Math.fround(e);if(!Number.isFinite(n))throw new A(`${t}: value ${e} is not representable as a finite f32 (WGSL kernels compute in f32; keep magnitudes within ~3.4e38)`);return Object.is(n,-0)?`-0f`:`${String(n)}f`}function ne(e){return`${e>>>0}u`}function M(e){return`0x${(e>>>0).toString(16).padStart(8,`0`)}u`}var N=M,re=j(34028234663852886e22,`internal f32 max`);function ie(e,t){let n=N(e);for(let e of t)n=`pcg_hash_mix(${n}, ${e})`;return`pcg_hash_finalize(${n})`}function ae(){let e=[];for(let t=0;t<12;t++){let n=e=>j(u[t*3+e],`internal GRAD3`);e.push(`  vec3<f32>(${n(0)}, ${n(1)}, ${n(2)}),`)}return`var<private> PCG_GRAD3: array<vec3<f32>, 12> = array<vec3<f32>, 12>(
${e.join(`
`)}
  ...
  return ${P(`internal PERLIN_SCALE`)(C)} * pcg_mix(
  ...
  let t = ${P(`internal simplex R2`)(x)} - x * x - y * y - z * z;
  if (t <= 0f) {
    return 0f;
  }
  ...
  let s = (p.x + p.y + p.z) * ${P(`internal simplex F3`)(f)};
  let i = i32(floor(p.x + s));
  let j = i32(floor(p.y + s));
  let k = i32(floor(p.z + s));
  let t = f32(i + j + k) * ${P(`internal simplex G3`)(k)};
  let x0 = p.x - (f32(i) - t);
  let y0 = p.y - (f32(j) - t);
  let z0 = p.z - (f32(k) - t);
  var i1: i32;
  ...
  let x1 = x0 - f32(i1) + ${P(`internal simplex G3`)(k)};
  let y1 = y0 - f32(j1) + ${P(`internal simplex G3`)(k)};
  let z1 = z0 - f32(k1) + ${P(`internal simplex G3`)(k)};
  let x2 = x0 - f32(i2) + ${P(`internal simplex 2*G3`)(2*k)};
  let y2 = y0 - f32(j2) + ${P(`internal simplex 2*G3`)(2*k)};
  let z2 = z0 - f32(k2) + ${P(`internal simplex 2*G3`)(2*k)};
  let x3 = x0 - 1f + ${P(`internal simplex 3*G3`)(3*k)};
  let y3 = y0 - 1f + ${P(`internal simplex 3*G3`)(3*k)};
  let z3 = z0 - 1f + ${P(`internal simplex 3*G3`)(3*k)};
