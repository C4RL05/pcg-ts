import{$n as e,Fr as t,Gn as n,Gr as r,Hn as i,Hr as a,In as o,Jr as s,Kn as c,Kr as l,Lr as u,Mn as d,Pr as f,Qn as p,Qr as m,Rr as h,Un as g,Ur as _,Vr as v,Vt as y,Wn as b,Wr as x,Xn as S,Xr as C,Yn as w,Yr as T,Zr as E,ar as D,er as O,qn as ee,qr as te,zr as k}from"./wordmark-CDO1K2E8.js";var A=class extends Error{constructor(e){super(e),this.name=`GpuCompileError`}};function j(e,t){let n=Math.fround(e);if(!Number.isFinite(n))throw new A(`${t}: value ${e} is not representable as a finite f32 (WGSL kernels compute in f32; keep magnitudes within ~3.4e38)`);return Object.is(n,-0)?`-0f`:`${String(n)}f`}function ne(e){return`${e>>>0}u`}function M(e){return`0x${(e>>>0).toString(16).padStart(8,`0`)}u`}var N=M,re=j(34028234663852886e22,`internal f32 max`);function ie(e,t){let n=N(e);for(let e of t)n=`pcg_hash_mix(${n}, ${e})`;return`pcg_hash_finalize(${n})`}function ae(){let e=[];for(let t=0;t<12;t++){let n=e=>j(p[t*3+e],`internal GRAD3`);e.push(`  vec3<f32>(${n(0)}, ${n(1)}, ${n(2)}),`)}return`var<private> PCG_GRAD3: array<vec3<f32>, 12> = array<vec3<f32>, 12>(
${e.join(`
`)}
  ...
  return ${P(`internal PERLIN_SCALE`)(S)} * pcg_mix(
  ...
  let t = ${P(`internal simplex R2`)(c)} - x * x - y * y - z * z;
  if (t <= 0f) {
    return 0f;
  }
  ...
  let s = (p.x + p.y + p.z) * ${P(`internal simplex F3`)(b)};
  let i = i32(floor(p.x + s));
  let j = i32(floor(p.y + s));
  let k = i32(floor(p.z + s));
  let t = f32(i + j + k) * ${P(`internal simplex G3`)(n)};
  let x0 = p.x - (f32(i) - t);
  let y0 = p.y - (f32(j) - t);
  let z0 = p.z - (f32(k) - t);
  var i1: i32;
  ...
  let x1 = x0 - f32(i1) + ${P(`internal simplex G3`)(n)};
  let y1 = y0 - f32(j1) + ${P(`internal simplex G3`)(n)};
  let z1 = z0 - f32(k1) + ${P(`internal simplex G3`)(n)};
  let x2 = x0 - f32(i2) + ${P(`internal simplex 2*G3`)(2*n)};
  let y2 = y0 - f32(j2) + ${P(`internal simplex 2*G3`)(2*n)};
  let z2 = z0 - f32(k2) + ${P(`internal simplex 2*G3`)(2*n)};
  let x3 = x0 - 1f + ${P(`internal simplex 3*G3`)(3*n)};
  let y3 = y0 - 1f + ${P(`internal simplex 3*G3`)(3*n)};
  let z3 = z0 - 1f + ${P(`internal simplex 3*G3`)(3*n)};
