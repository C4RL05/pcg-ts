...
`)}
  ...
  return f32(h >> 8u) * ${j(l,`internal hashFloat scale`)};
  ...
  return ${P(`internal PERLIN_SCALE`)(w)} * pcg_mix(
  ...
  let t = ${P(`internal simplex R2`)(S)} - x * x - y * y - z * z;
  if (t <= 0f) {
    return 0f;
  }
  ...
  let s = (p.x + p.y + p.z) * ${P(`internal simplex F3`)(p)};
