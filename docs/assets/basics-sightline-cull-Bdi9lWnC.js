var e=`{
  ...
  "seed": 3391,
  "meta": {
    "title": "clear a line of sight by moving the props, not by deleting them",
  ...
  "nodes": [
    {
      "id": "loop",
      "type": "subgraph",
      "params": { "count": 72, "size": [26, 1, 26] },
      "ref": { "name": "shape/path-loop" }
    },
    {
      "id": "eyes",
      "type": "pathResample",
      "params": { "mode": "spacing", "spacing": 3 }
    },
    {
      "id": "props",
    ...
    },
    {
      ...
      "params": {
        ...
        "type": "f32",
        "tupleSize": 3,
        "value": { "fn": "constant", "value": [3.2, 3.6, 1.0] }
      }
    },
    {
      "id": "drop",
    ...
    { "id": "roads", "type": "mergePrimitives", "params": {} },
    {
      "id": "spawnDrop",
      "type": "spawnInstances",
  ...
  "connections": [
    { "from": ["loop", "out"], "to": ["eyes", "in"] },
    { "from": ["props", "out"], "to": ["yaw", "in"] },
    { "from": ["yaw", "out"], "to": ["hoarding", "in"] },

    { "from": ["hoarding", "out"], "to": ["drop", "in"] },
    { "from": ["eyes", "out"], "to": ["drop", "sight"] },
    { "from": ["hoarding", "out"], "to": ["push", "in"] },
    { "from": ["eyes", "out"], "to": ["push", "sight"] },

    { "from": ["drop", "out"], "to": ["dropShift", "in"] },
    { "from": ["eyes", "out"], "to": ["roadLeft", "in"] },
    { "from": ["roadLeft", "out"], "to": ["roads", "in"] },
    { "from": ["eyes", "out"], "to": ["roads", "in"] },

    { "from": ["dropShift", "out"], "to": ["spawnDrop", "in"] },
    { "from": ["push", "out"], "to": ["spawnPush", "in"] }
  ],
  "outputs": [
    { "id": "roads", "pin": "out", "name": "roads" },
    { "id": "spawnDrop", "pin": "instances", "name": "dropped" },
    { "id": "spawnPush", "pin": "instances", "name": "pushed" }
  ]
}
`;export{e as default};