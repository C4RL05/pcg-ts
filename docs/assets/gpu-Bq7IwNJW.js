...
`)}
  ...
  return ${P(`internal PERLIN_SCALE`)(e)} * pcg_mix(
  ...
  let s = (p.x + p.y + p.z) * ${P(`internal simplex F3`)(l)};
...
${g.join(`
`)}`,...se(s.libRoots),...s.helperBlocks(),`@compute @workgroup_size(${Ce})
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  let i = gid.x + params.chunkOffset;
  if (i >= params.count) {
    return;
