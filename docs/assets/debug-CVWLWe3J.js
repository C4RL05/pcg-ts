import{$r as e,An as t,Bn as n,Bt as r,Gn as i,Gr as a,Hn as o,Hr as s,Ht as c,Jn as l,Jr as u,Kr as d,Pn as f,Qn as p,Qr as m,Un as h,Vn as g,Vr as _,Vt as v,Wn as y,Wr as b,Xn as x,Xr as S,Yr as C,Zn as w,Zr as ee,ai as T,ei as E,ii as D,ni as O,qn as te,ri as k,rr as ne,ti as re}from"./wordmark-Be0gqpu1.js";import{Q as ie,Ut as ae,Wt as oe,X as se,o as ce,s as le}from"./three.core-B85ZZh_6.js";var A=class extends Error{constructor(e){super(e),this.name=`GpuCompileError`}};function j(e,t){let n=Math.fround(e);if(!Number.isFinite(n))throw new A(`${t}: value ${e} is not representable as a finite f32 (WGSL kernels compute in f32; keep magnitudes within ~3.4e38)`);return Object.is(n,-0)?`-0f`:`${String(n)}f`}function ue(e){return`${e>>>0}u`}function M(e){return`0x${(e>>>0).toString(16).padStart(8,`0`)}u`}var N=M,de=j(34028234663852886e22,`internal f32 max`);function fe(e,t){let n=N(e);for(let e of t)n=`pcg_hash_mix(${n}, ${e})`;return`pcg_hash_finalize(${n})`}function pe(){let e=[];for(let t=0;t<12;t++){let n=e=>j(x[t*3+e],`internal GRAD3`);e.push(`  vec3<f32>(${n(0)}, ${n(1)}, ${n(2)}),`)}return`var<private> PCG_GRAD3: array<vec3<f32>, 12> = array<vec3<f32>, 12>(
${e.join(`
`)}
  ...
  return ${P(`internal PERLIN_SCALE`)(l)} * pcg_mix(
  ...
  let s = (p.x + p.y + p.z) * ${P(`internal simplex F3`)(o)};
  let i = i32(floor(p.x + s));
  let j = i32(floor(p.y + s));
  let k = i32(floor(p.z + s));
  let t = f32(i + j + k) * ${P(`internal simplex G3`)(h)};
  let x0 = p.x - (f32(i) - t);
  let y0 = p.y - (f32(j) - t);
  let z0 = p.z - (f32(k) - t);
  var i1: i32;
  ...
  let x1 = x0 - f32(i1) + ${P(`internal simplex G3`)(h)};
  let y1 = y0 - f32(j1) + ${P(`internal simplex G3`)(h)};
  let z1 = z0 - f32(k1) + ${P(`internal simplex G3`)(h)};
  let x2 = x0 - f32(i2) + ${P(`internal simplex 2*G3`)(2*h)};
  let y2 = y0 - f32(j2) + ${P(`internal simplex 2*G3`)(2*h)};
  let z2 = z0 - f32(k2) + ${P(`internal simplex 2*G3`)(2*h)};
  let x3 = x0 - 1f + ${P(`internal simplex 3*G3`)(3*h)};
  let y3 = y0 - 1f + ${P(`internal simplex 3*G3`)(3*h)};
  let z3 = z0 - 1f + ${P(`internal simplex 3*G3`)(3*h)};
