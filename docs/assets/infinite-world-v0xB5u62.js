import{An as e,D as t,En as n,Fn as r,Gr as i,It as a,Kr as o,Lr as ee,Rt as te,Un as ne,Vr as s,Xn as re,Yr as ie,di as c,hn as ae,hr as oe,kn as se,mn as ce,mr as le,or as ue,r as l,yr as de}from"./wordmark-El-7dZE6.js";import{t as fe}from"./types-C9vZymIy.js";import{A as u,St as d,U as f,Vt as p,W as m,jt as h,q as g}from"./three.core-B85ZZh_6.js";import{t as _}from"./scene-QpK-6DxV.js";import{t as pe}from"./gpu-BSIWsI5c.js";import{t as me}from"./worldBinding-CV8N6Vcb.js";import{n as he,t as ge}from"./panel-DyHVwwB7.js";var _e=.35,ve=777;function ye(){let e=new ne,t=e.add(ce,{count:170,boundsMin:[-800,0,-800],boundsMax:[800,0,800]}),n=e.add(r,{name:`scale`,tupleSize:3,value:(()=>{let e=s(ie(`mega`),0,1,5,13);return i(e,ee(e,1.6),e)})()}),a=e.add(te,{assetId:`megarock`});return e.connect(t,`out`,n,`in`),e.connect(n,`out`,a,`in`),e.output(a,`instances`,`instances`),{name:`landmarks`,cellSize:`unbounded`,generationRadius:1/0,graph:e,bind(e,r){e.setParam(t,`seed`,r.seed),e.setParam(n,`seed`,c(r.seed,9))}}}function be(t){let{cellSize:a,generationRadius:oe,anchored:le}=t,l=t.halo?4:0,u=new ne,d=le?u.add(ae,{density:_e,cellSize:7,latticeMode:`xz`,height:0}):u.add(ce,{count:Math.round(_e*(a+2*l)**2)}),f=u.add(r,{name:`density`,tupleSize:1,value:s(re(ue,{seed:ve,frequency:.02,octaves:3}),-1,1,0,1)}),p=u.add(e,{mode:`probabilistic`}),m=u.add(n,{radius:4,countAttr:`nbrCount`}),h=u.add(se,{mode:`inside`}),g=u.add(r,{name:`scale`,tupleSize:3,value:(()=>{let e=s(de(o(`nbrCount`),2,16),2,16,.4,1.35),t=ee(e,s(ie(`rock`),0,1,.85,1.15));return i(t,t,t)})()}),_=u.add(te,{assetId:`rock`});return u.connect(d,`out`,f,`in`),u.connect(f,`out`,p,`in`),u.connect(p,`out`,m,`in`),u.connect(m,`out`,h,`in`),u.connect(h,`out`,g,`in`),u.connect(g,`out`,_,`in`),u.output(_,`instances`,`instances`),{name:`rocks`,cellSize:a,generationRadius:oe,graph:u,bind(e,t){let{min:n,max:r}=fe(t);e.setParam(d,`boundsMin`,[n[0]-l,0,n[1]-l]),e.setParam(d,`boundsMax`,[r[0]+l,0,r[1]+l]);let i=le?t.worldSeed:t.seed;e.setParam(d,`seed`,i),e.setParam(p,`seed`,c(i,1)),e.setParam(g,`seed`,c(i,2)),e.setParam(h,`boundsMin`,[n[0],-1,n[1]]),e.setParam(h,`boundsMax`,[r[0],1,r[1]])}}}function xe(e){return e instanceof Error?e.message:String(e)}function Se(e){let t=(e?.description??``)===``?e?.device:e?.description;return[e?.vendor,e?.architecture,t].filter(e=>typeof e==`string`&&e!==``).join(` · `)||`adapter (no info exposed)`}async function Ce(e){let t=navigator.gpu;if(t===void 0)return{error:`navigator.gpu is missing — this browser has no WebGPU.`};try{let n=await t.requestAdapter();if(n===null)return{error:`navigator.gpu.requestAdapter() returned null — no compatible GPU adapter.`};let r=n.info,i=await n.requestDevice(),a=i.lost;return a!==void 0&&a.then(t=>{e(`${t?.reason??`unknown`}: ${t?.message??`no detail`}`)}),{device:i,info:r,label:Se(r)}}catch(e){return{error:`requestDevice() failed: ${xe(e)}`}}}function we(e){let t=e.info===void 0?{}:{adapterInfo:e.info};return{derived:new pe(e.device,{...t,acceptDerivedSpecs:!0}),strict:new pe(e.device,t)}}var Te=class{cacheSalt;residentTerminals;acceptDerivedSpecs;base;seen=new Set;constructor(e){this.base=e,this.cacheSalt=e.cacheSalt,this.residentTerminals=e.residentTerminals,this.acceptDerivedSpecs=e.acceptDerivedSpecs}resolveField(e,t,n){return n!==void 0&&this.seen.add(n),this.base.resolveField(e,t,n)}planRun(e,t,n){return n!==void 0&&this.seen.add(n),this.base.planRun(e,t,n)}executeRun(e,t,n){return n!==void 0&&this.seen.add(n),this.base.executeRun(e,t,n)}drain(){if(this.seen.size===0)return;let e=oe();for(let t of this.seen){e.dispatches+=t.dispatches,e.pipelinesCompiled+=t.pipelinesCompiled,e.pipelineCacheHits+=t.pipelineCacheHits,e.residentRuns+=t.residentRuns,e.fusedNodes+=t.fusedNodes,e.readbacksSaved+=t.readbacksSaved;for(let[n,r]of Object.entries(t.fallbacks))e.fallbacks[n]=(e.fallbacks[n]??0)+r}return this.seen.clear(),e}};function Ee(e){let t=Object.entries(e).sort(([e],[t])=>e.localeCompare(t));return t.length===0?`none`:t.map(([e,t])=>`${e} ×${t}`).join(`, `)}var v=1,y=18,b=140,x=`gpu`,S=!0,C=20,w=!0,T=!0,E=!0,De=7,Oe=12,ke=8,Ae=5e3;function je(e){return e instanceof Error?e.message:String(e)}var{scene:D,camera:O,start:Me}=_({cameraPosition:[0,26,0],orbit:!1,fog:{near:140,far:640},far:1600}),Ne=new d(new p(4e3,4e3),new h({color:1449515,roughness:1}));Ne.rotation.x=-Math.PI/2,Ne.position.y=-.03,D.add(Ne);var Pe=40,k;function Fe(){k&&(D.remove(k),k.geometry.dispose(),k.material.dispose());let e=new f(C*Pe,Pe,4158656,2903936),t=e.material;t.transparent=!0,t.opacity=.45,t.depthWrite=!1,e.position.y=.02,e.visible=E,D.add(e),k=e}function Ie(){!k||!k.visible||(k.position.x=Math.round(O.position.x/C)*C,k.position.z=Math.round(O.position.z/C)*C)}var Le=new h({color:8028816,roughness:.95,flatShading:!0}),Re=new h({color:6121077,roughness:1,flatShading:!0}),ze={megarock:{geometry:new u(1).translate(0,.62,0),material:Re},rock:{geometry:new g(.55).translate(0,.33,0),material:Le}},Be,Ve,A=`requesting…`,j,M=!1,He=``,N;function P(){return Be!==void 0&&!M}function Ue(){if(!(x===`cpu`||!P()))return S?Be:Ve}var F;function We(e,t){let n=[{name:`landmarks`,graph:e},{name:`rocks`,graph:t}];F?F.set(n):F=ge(n,{into:_t,title:`infinite world`})}function Ge(e,t){let n=e*1.25/t+1.5;return Math.max(256,Math.ceil(Math.PI*n*n))}function Ke(e){let t=0;for(let n of Object.keys(e))for(let r of e[n])if(r.kind===`instances`)for(let e of r.batches)t+=e.count;return t}var I,L=0,R,z=0,B=!1,qe=0;function V(){I&&(I.disposed=!0,I.abort.abort(),I.binding.dispose(),D.remove(I.group));let e=new m;D.add(e);let t=new me({group:e,assets:ze}),n=new Map,r=Ue(),i={world:void 0,group:e,binding:t,cellInstances:n,tap:r===void 0?void 0:new Te(r),abort:new AbortController,disposed:!1},o=[ye(),be({cellSize:C,generationRadius:b,anchored:w,halo:T})];We(o[0].graph,o[1].graph),i.world=new a({seed:v,levels:o,maxCellsPerLevel:Ge(b,C),...i.tap===void 0?{}:{gpu:i.tap},onCellReady:(e,r,a)=>{i.disposed||(t.cellReady(e,r,a),n.set(`${e}|${r[0]},${r[1]}`,Ke(a)))},onCellEvicted:(e,r)=>{i.disposed||(t.cellEvicted(e,r),n.delete(`${e}|${r[0]},${r[1]}`))}}),I=i,L=0,N=void 0,R=void 0,gt(),Je(i,Oe)}function Je(e,t){let n=++qe;$=!0,z=performance.now(),e.world.update([O.position.x,0,O.position.z],{budgetMs:t,maxCooksPerUpdate:ke,signal:e.abort.signal,...e.tap===void 0?{}:{gpu:e.tap}}).then(t=>{if(e.disposed)return;L=t.pending,R=t.elapsedMs;let n=e.tap?.drain();n!==void 0&&(N=n),gt(),B&&(B=!1,Z(ht()))}).catch(t=>{t instanceof le||(console.error(t),e.disposed||Z(`update failed: ${je(t)}`))}).finally(()=>{n===qe&&($=!1)})}var H=0,Ye=0,U=new Set;window.addEventListener(`keydown`,e=>{let t=e.key.toLowerCase();if(t===`g`){tt(x===`gpu`?`cpu`:`gpu`);return}U.add(t)}),window.addEventListener(`keyup`,e=>U.delete(e.key.toLowerCase())),document.addEventListener(`visibilitychange`,()=>{document.visibilityState===`visible`&&(z=performance.now())});var W=he({title:`infinite world`,info:`Unbounded landmark level + world-anchored rock cells streamed around a flying camera. Steer with A/D or arrow keys.`}),Xe=document.createElement(`style`);Xe.textContent=`
...
`,document.head.appendChild(Xe);var Ze=W.el.querySelector(`.pcg-stats`)?.previousElementSibling??W.el;W.addSeed(v,e=>{v=e,V()}),W.addSlider(`speed`,{min:0,max:80,step:1,value:y,format:e=>`${e} u/s`},e=>{y=e}),W.addSlider(`gen radius`,{min:60,max:240,step:10,value:b,format:e=>`${e} u`},e=>{b=e,V()}),W.addSlider(`cell size`,{min:20,max:80,step:20,value:C,format:e=>`${e} u`},e=>{C=e,Fe(),V()}),W.addCheckbox(`world-anchored`,w,e=>{w=e,mt(),V()}),W.addCheckbox(`halo (4 u)`,T,e=>{T=e,V()}),W.addCheckbox(`cell grid`,E,e=>{E=e,k&&(k.visible=e),Ie()});var G=document.createElement(`div`);G.className=`pcg04-seg`;var K=document.createElement(`button`);K.type=`button`,K.textContent=`GPU per-node`;var q=document.createElement(`button`);q.type=`button`,q.textContent=`CPU`,G.append(K,q);var Qe=document.createElement(`p`);Qe.className=`pcg04-hint`,Qe.textContent=`G switches path · A/D or ←/→ steer`;var $e=document.createElement(`div`);$e.className=`pcg-row`;var et=document.createElement(`label`);et.textContent=`derived specs`;var J=document.createElement(`input`);J.type=`checkbox`,J.checked=S,J.addEventListener(`change`,()=>{S=J.checked,V()}),$e.append(et,J),Ze.prepend(G,Qe,$e);function tt(e){if(e!==x){if(e===`gpu`&&!P()){Z(M?Q():j??`GPU path not ready yet.`);return}x=e,Y(),V(),Z(ht())}}K.addEventListener(`click`,()=>tt(`gpu`)),q.addEventListener(`click`,()=>tt(`cpu`));function Y(){K.setAttribute(`aria-pressed`,String(x===`gpu`)),q.setAttribute(`aria-pressed`,String(x===`cpu`)),K.disabled=!P(),K.title=j===void 0?M?Q():``:j,J.disabled=!P()||x===`cpu`}var X=W.addStat(`adapter`),nt=W.addStat(`fps`),rt=W.addStat(`rock source`),it=W.addStat(`rock cells`),at=W.addStat(`cooked / evicted`),ot=W.addStat(`pending`),st=W.addStat(`instances`),ct=W.addStat(`position`),lt=W.addStat(`cook`),ut=W.addStat(`resident runs / fused members`),dt=W.addStat(`device dispatches`),ft=W.addStat(`gpu fallbacks`),pt=W.addStat(`status`);function Z(e){pt(e)}function mt(){rt(w?`pointScatterInWorld`:`pointScatterInBounds`)}function Q(){return`device lost (${He}) — the GPU path is disabled; reload for a fresh device`}function ht(){return M?Q():x===`cpu`?`CPU path — no resolver passed to the cook`:P()?S?`GPU per-node — combinator fields accepted via acceptDerivedSpecs`:`GPU per-node — acceptDerivedSpecs off, so every field falls back`:j??`requesting WebGPU adapter…`}function gt(){if(lt(R===void 0?`–`:`${R.toFixed(1)} ms`),N===void 0){ut(I?.tap===void 0?`– (no GPU resolver)`:`–`),dt(`–`),ft(`–`);return}ut(`${N.residentRuns} / ${N.fusedNodes}`),dt(String(N.dispatches)),ft(Ee(N.fallbacks))}var _t=W.addSlot();l(),W.addNote(`Drag “cell size”: the blue grid re-cells the world and the rocks do not move or resize. Untick “halo” to watch every border grow a band of undersized rocks, and “world-anchored” to watch the same drag re-roll the world from scratch.`);var vt=W.addCollapsible(`anchoring · what the fine level actually does`);vt.textContent=`pointScatterInWorld scatters over an INFINITE lattice fixed to
...
update cooks nothing.`;function bt(e){M=!0,He=e,A=`device lost`,X(A),Z(Q()),Y(),x===`gpu`&&(x=`cpu`,Y(),V())}async function xt(){let e=await Ce(bt);if(`error`in e){j=e.error,A=`no WebGPU adapter`,X(A),x=`cpu`,Y(),Z(`${e.error} Running the CPU path.`);return}try{let t=we(e);Be=t.derived,Ve=t.strict}catch(e){j=`GpuFieldEvaluator construction failed: ${je(e)}`,A=`evaluator unavailable`,X(A),x=`cpu`,Y(),Z(`${j} Running the CPU path.`);return}M||(A=e.label,X(A),Y(),Z(ht()),x===`gpu`&&V())}var $=!1,St=t(e=>nt(e));X(A),Z(`requesting WebGPU adapter…`),Y(),mt(),gt(),Fe(),V(),xt(),Me(e=>{St(),Ye=(U.has(`a`)||U.has(`arrowleft`)?1:0)-(U.has(`d`)||U.has(`arrowright`)?1:0),H+=Ye*1.1*e,O.position.x+=Math.sin(H)*y*e,O.position.z+=Math.cos(H)*y*e,O.position.y=26,O.lookAt(O.position.x+Math.sin(H)*60,5,O.position.z+Math.cos(H)*60),Ie();let t=I;if(t&&!$?Je(t,De):$&&!B&&performance.now()-z>Ae&&(B=!0,Z(`a cook has been in flight for over 5 s — the GPU device may have been lost.`)),t){let e=t.world.stats(),n=e.levels.find(e=>e.name===`rocks`)?.cellCount??0;it(String(n)),at(`${e.totalCooked} / ${e.totalEvicted}`),ot(String(L));let r=0;for(let e of t.cellInstances.values())r+=e;st(String(r)),ct(`${O.position.x.toFixed(0)}, ${O.position.z.toFixed(0)}`)}});