var e=`{\r
  "formatVersion": 1,\r
  "seed": 1018,\r
  "meta": {\r
    "title": "measure how crowded each point is",\r
    "description": "\`pointNeighborhood\` writes how many other points lie within \`radius\` into a u32 attribute, using a uniform spatial grid so it stays fast well beyond a few thousand points. The count is a measured quantity rather than an authored one, which is what a later filter, colour or scale can react to. A point with no neighbours gets 0 and keeps its own value as the neighbour average, so a displacement built from that average is zero rather than undefined.",\r
    "tags": ["basics", "attributes", "neighborhood", "measure"]\r
  },\r
  "nodes": [\r
    {\r
      "id": "scatter",\r
      "type": "pointScatterInBounds",\r
      "params": {\r
        "count": 700,\r
        "boundsMin": [-25, 0, -25],\r
        "boundsMax": [25, 0, 25]\r
      }\r
    },\r
    {\r
      "id": "crowding",\r
      "type": "pointNeighborhood",\r
      "params": { "radius": 4, "countAttr": "nbrCount" }\r
    }\r
  ],\r
  "connections": [{ "from": ["scatter", "out"], "to": ["crowding", "in"] }],\r
  "outputs": [{ "id": "crowding", "pin": "out", "name": "points" }]\r
}\r
`;export{e as default};