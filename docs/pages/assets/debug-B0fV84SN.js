import{$ as e,Ja as t,Ji as n,Jr as r,Kt as i,Li as a,Qa as o,Qi as s,Vi as c,Xa as l,Xi as u,Yi as d,Yr as f,Za as p,Zi as m,aa as h,ao as g,co as _,do as v,ea as y,eo as b,ia as x,io as S,l as C,la as w,lo as T,no as E,oo as D,qa as O,qi as ee,qr as te,qt as k,ra as ne,ro as re,so as ie,ta as ae,to as oe,tt as se,u as ce,uo as le}from"./mobile-CJx-ChGw.js";var A=class extends Error{constructor(e){super(e),this.name=`GpuCompileError`}};function j(e,t){let n=Math.fround(e);if(!Number.isFinite(n))throw new A(`${t}: value ${e} is not representable as a finite f32 (WGSL kernels compute in f32; keep magnitudes within ~3.4e38)`);return Object.is(n,-0)?`-0f`:`${String(n)}f`}function ue(e){return`${e>>>0}u`}function M(e){return`0x${(e>>>0).toString(16).padStart(8,`0`)}u`}var N=M,de=j(34028234663852886e22,`internal f32 max`);function fe(e,t){let n=N(e);for(let e of t)n=`pcg_hash_mix(${n}, ${e})`;return`pcg_hash_finalize(${n})`}function pe(){let e=[];for(let t=0;t<12;t++){let n=e=>j(ne[t*3+e],`internal GRAD3`);e.push(`  vec3<f32>(${n(0)}, ${n(1)}, ${n(2)}),`)}return`var<private> PCG_GRAD3: array<vec3<f32>, 12> = array<vec3<f32>, 12>(
${e.join(`
`)}
  ...
  return f32(h >> 8u) * ${j(S,`internal hashFloat scale`)};
  ...
  return ${P(`internal PERLIN_SCALE`)(ae)} * pcg_mix(
  ...
  let t = f32(i + j + k) * ${P(`internal simplex G3`)(u)};
  let x0 = p.x - (f32(i) - t);
  let y0 = p.y - (f32(j) - t);
  let z0 = p.z - (f32(k) - t);
  var i1: i32;
  ...
  let x1 = x0 - f32(i1) + ${P(`internal simplex G3`)(u)};
  let y1 = y0 - f32(j1) + ${P(`internal simplex G3`)(u)};
  let z1 = z0 - f32(k1) + ${P(`internal simplex G3`)(u)};
  let x2 = x0 - f32(i2) + ${P(`internal simplex 2*G3`)(2*u)};
  let y2 = y0 - f32(j2) + ${P(`internal simplex 2*G3`)(2*u)};
  let z2 = z0 - f32(k2) + ${P(`internal simplex 2*G3`)(2*u)};
  let x3 = x0 - 1f + ${P(`internal simplex 3*G3`)(3*u)};
  let y3 = y0 - 1f + ${P(`internal simplex 3*G3`)(3*u)};
  let z3 = z0 - 1f + ${P(`internal simplex 3*G3`)(3*u)};
