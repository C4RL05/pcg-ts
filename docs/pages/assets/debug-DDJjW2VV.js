import{Ai as e,Bi as t,Fi as n,Gr as r,Gt as i,Hi as a,Kr as o,Li as s,Mi as c,Ni as l,Pi as u,Q as d,Ri as f,Sa as p,Ta as m,Ti as h,Vi as g,Wr as _,Wt as v,_a as y,aa as b,ba as x,ca as S,da as C,et as ee,fa as w,ga as T,ha as E,ji as D,l as O,la as k,ma as te,oa as ne,pa as re,u as ie,ua as ae,va as oe,xa as se,xi as ce,ya as le}from"./mobile-Bib_DZ1z.js";var A=class extends Error{constructor(e){super(e),this.name=`GpuCompileError`}};function j(e,t){let n=Math.fround(e);if(!Number.isFinite(n))throw new A(`${t}: value ${e} is not representable as a finite f32 (WGSL kernels compute in f32; keep magnitudes within ~3.4e38)`);return Object.is(n,-0)?`-0f`:`${String(n)}f`}function ue(e){return`${e>>>0}u`}function M(e){return`0x${(e>>>0).toString(16).padStart(8,`0`)}u`}var N=M,de=j(34028234663852886e22,`internal f32 max`);function fe(e,t){let n=N(e);for(let e of t)n=`pcg_hash_mix(${n}, ${e})`;return`pcg_hash_finalize(${n})`}function pe(){let e=[];for(let n=0;n<12;n++){let r=e=>j(t[n*3+e],`internal GRAD3`);e.push(`  vec3<f32>(${r(0)}, ${r(1)}, ${r(2)}),`)}return`var<private> PCG_GRAD3: array<vec3<f32>, 12> = array<vec3<f32>, 12>(
${e.join(`
`)}
  ...
  return ${P(`internal PERLIN_SCALE`)(f)} * pcg_mix(
  ...
  let t = ${P(`internal simplex R2`)(u)} - x * x - y * y - z * z;
  if (t <= 0f) {
    return 0f;
  }
  ...
  let t = f32(i + j + k) * ${P(`internal simplex G3`)(l)};
  let x0 = p.x - (f32(i) - t);
  let y0 = p.y - (f32(j) - t);
  let z0 = p.z - (f32(k) - t);
  var i1: i32;
  ...
  let x1 = x0 - f32(i1) + ${P(`internal simplex G3`)(l)};
  let y1 = y0 - f32(j1) + ${P(`internal simplex G3`)(l)};
  let z1 = z0 - f32(k1) + ${P(`internal simplex G3`)(l)};
  let x2 = x0 - f32(i2) + ${P(`internal simplex 2*G3`)(2*l)};
  let y2 = y0 - f32(j2) + ${P(`internal simplex 2*G3`)(2*l)};
  let z2 = z0 - f32(k2) + ${P(`internal simplex 2*G3`)(2*l)};
  let x3 = x0 - 1f + ${P(`internal simplex 3*G3`)(3*l)};
  let y3 = y0 - 1f + ${P(`internal simplex 3*G3`)(3*l)};
  let z3 = z0 - 1f + ${P(`internal simplex 3*G3`)(3*l)};
