import{Bi as e,Ca as t,Da as n,Ea as r,Ei as i,Fi as a,Gr as o,Gt as s,Hi as c,Ii as l,Kr as u,Kt as d,Li as f,Mi as p,Ni as m,Oa as h,Pi as g,Q as _,Sa as v,Si as y,Ta as b,Ui as x,Wi as S,_a as C,ba as w,da as T,et as E,ha as D,l as O,ma as k,pa as ee,qi as A,qr as te,u as ne,ua as re,va as ie,wa as ae,xa as oe,ya as se,zi as ce}from"./mobile-DA60hz68.js";var j=class extends Error{constructor(e){super(e),this.name=`GpuCompileError`}};function M(e,t){let n=Math.fround(e);if(!Number.isFinite(n))throw new j(`${t}: value ${e} is not representable as a finite f32 (WGSL kernels compute in f32; keep magnitudes within ~3.4e38)`);return Object.is(n,-0)?`-0f`:`${String(n)}f`}function le(e){return`${e>>>0}u`}function N(e){return`0x${(e>>>0).toString(16).padStart(8,`0`)}u`}var P=N,ue=M(34028234663852886e22,`internal f32 max`);function de(e,t){let n=P(e);for(let e of t)n=`pcg_hash_mix(${n}, ${e})`;return`pcg_hash_finalize(${n})`}function fe(){let e=[];for(let t=0;t<12;t++){let n=e=>M(c[t*3+e],`internal GRAD3`);e.push(`  vec3<f32>(${n(0)}, ${n(1)}, ${n(2)}),`)}return`var<private> PCG_GRAD3: array<vec3<f32>, 12> = array<vec3<f32>, 12>(
${e.join(`
`)}
  ...
  let t = ${F(`internal simplex R2`)(l)} - x * x - y * y - z * z;
  if (t <= 0f) {
    return 0f;
  }
  ...
  let s = (p.x + p.y + p.z) * ${F(`internal simplex F3`)(g)};
  let i = i32(floor(p.x + s));
  let j = i32(floor(p.y + s));
  let k = i32(floor(p.z + s));
  let t = f32(i + j + k) * ${F(`internal simplex G3`)(a)};
  let x0 = p.x - (f32(i) - t);
  let y0 = p.y - (f32(j) - t);
  let z0 = p.z - (f32(k) - t);
  var i1: i32;
  ...
  let x1 = x0 - f32(i1) + ${F(`internal simplex G3`)(a)};
  let y1 = y0 - f32(j1) + ${F(`internal simplex G3`)(a)};
  let z1 = z0 - f32(k1) + ${F(`internal simplex G3`)(a)};
  let x2 = x0 - f32(i2) + ${F(`internal simplex 2*G3`)(2*a)};
  let y2 = y0 - f32(j2) + ${F(`internal simplex 2*G3`)(2*a)};
  let z2 = z0 - f32(k2) + ${F(`internal simplex 2*G3`)(2*a)};
  let x3 = x0 - 1f + ${F(`internal simplex 3*G3`)(3*a)};
  let y3 = y0 - 1f + ${F(`internal simplex 3*G3`)(3*a)};
  let z3 = z0 - 1f + ${F(`internal simplex 3*G3`)(3*a)};
