...
`)}
  ...
  return ${P(`internal PERLIN_SCALE`)(ce)} * pcg_mix(
  ...
  let t = ${P(`internal simplex R2`)(m)} - x * x - y * y - z * z;
  if (t <= 0f) {
    return 0f;
  }
  ...
  let s = (p.x + p.y + p.z) * ${P(`internal simplex F3`)(S)};
  let i = i32(floor(p.x + s));
  let j = i32(floor(p.y + s));
  let k = i32(floor(p.z + s));
  let t = f32(i + j + k) * ${P(`internal simplex G3`)(y)};
  let x0 = p.x - (f32(i) - t);
  let y0 = p.y - (f32(j) - t);
  let z0 = p.z - (f32(k) - t);
  var i1: i32;
  ...
  let x1 = x0 - f32(i1) + ${P(`internal simplex G3`)(y)};
  let y1 = y0 - f32(j1) + ${P(`internal simplex G3`)(y)};
  let z1 = z0 - f32(k1) + ${P(`internal simplex G3`)(y)};
  let x2 = x0 - f32(i2) + ${P(`internal simplex 2*G3`)(2*y)};
  let y2 = y0 - f32(j2) + ${P(`internal simplex 2*G3`)(2*y)};
  let z2 = z0 - f32(k2) + ${P(`internal simplex 2*G3`)(2*y)};
  let x3 = x0 - 1f + ${P(`internal simplex 3*G3`)(3*y)};
  let y3 = y0 - 1f + ${P(`internal simplex 3*G3`)(3*y)};
  let z3 = z0 - 1f + ${P(`internal simplex 3*G3`)(3*y)};
...
${`literal`in p?``:`  let ns = ${p.expr};\n`}  var seeds = array<u32, ${a}>(${_.join(`, `)});
  ...
  for (var o = 0u; o < ${ue(a)}; o++) {
    sum = sum + ${nt(r,`seeds[o]`,`p * freqs[o] + `+w)} * amps[o];
  }
  return sum;
}`,E=n.helper(`fbm`,T),D=n.emit(`${E}(${h.ref})`,1);if(i.normalized!==!0)return D;if(!(C>S))throw new A(`${t}: fbm: normalized: true needs a non-degenerate output range, got [${S}, ${C}] for this octaves/gain configuration`);return et(n,D,[S,C],`${t}.opts.normalized`)});var rt=new Set([`valueNoise`,`perlinNoise`,`simplexNoise`,`worleyNoise`,`fbm`]);function Y(e,t){if(!z(e))return;let n=e.fn;if(n===`param`){let n=h(e);n!==void 0&&Y(n,t);return}if(n===`attribute`||n===`attributeIs`){typeof e.name==`string`&&t.add(e.name);return}if(n===`byAttribute`){typeof e.name==`string`&&t.add(e.name);for(let n of _(e))Y(n,t);return}if(n===`position`){t.add(`P`);return}if(n===`randomField`){t.add(`P`),t.add(`seed`);return}if(typeof n==`string`&&rt.has(n)){let n=e.opts;z(n)&&n.position!==void 0?Y(n.position,t):t.add(`P`);return}let r=e.args;if(Array.isArray(r))for(let e of r)Y(e,t)}var it=16;function X(e,t){return`${JSON.stringify(e)},${JSON.stringify(t)}`}var at={names:[],slots:new Map,arities:[],attrIs:[],attrIsSlots:new Map};function ot(e){return typeof e==`number`?1:e.length}function st(e,t){if(z(e)){if(t(e),e.fn===`param`){let n=h(e);n!==void 0&&st(n,t);return}for(let n of _(e))st(n,t)}}function ct(e){let t=ut.get(e);if(t!==void 0)return t;let n=dt.get(e);if(n!==void 0)throw n;try{let t=lt(e);return ut.set(e,t),t}catch(t){throw t instanceof A&&dt.set(e,t),t}}function lt(e){let t=new Map,n=new Set,r=new Map;if(st(e,e=>{if(e.fn===`attributeIs`){if(typeof e.name!=`string`||e.name===``||typeof e.value!=`string`)return;r.set(X(e.name,e.value),{attr:e.name,value:e.value});return}if(e.fn===`byAttribute`){if(typeof e.name!=`string`||e.name===``||!z(e.cases))return;for(let t of Object.keys(e.cases))r.set(X(e.name,t),{attr:e.name,value:t});return}if(e.fn!==`param`||typeof e.name!=`string`||e.name===``)return;let i=e.name;if(h(e)!==void 0)return;if(b(e))throw new A(`param ${JSON.stringify(i)} is bound to a Field that carries no spec (a makeField closure, or something composed over one), so there is nothing to compile in its place; this expression evaluates on the CPU — build the bound field with the grammar constructors or fieldFromJson if it should lower`);n.add(i);let a=oe(e);if(a===void 0)return;let o=ot(a);if(o>4)throw new A(`param ${JSON.stringify(i)} is bound to a ${o}-tuple, but a uniform slot holds 4 components; bind a tuple of 1 to 4, or evaluate this field on the CPU`);let s=t.get(i);if(s!==void 0&&s!==o)throw new A(`param ${JSON.stringify(i)} is bound to a ${s}-tuple in one place and a ${o}-tuple in another within the same expression; one uniform slot serves the name, so both references must have the same arity`);t.set(i,o)}),n.size===0&&r.size===0)return at;let i=[...n].sort(),a=[...r.keys()].sort(),o=i.length+a.length;if(o>it)throw new A(`this field needs ${o} uniform constant slots (${i.length} distinct params and ${a.length} distinct string literals across its attributeIs tests and byAttribute case keys), but a kernel carries at most ${it}; split the expression, or evaluate it on the CPU (raise MAX_FIELD_CONST_SLOTS in compile.ts if an expression legitimately needs more)`);return{names:i,slots:new Map(i.map((e,t)=>[e,t])),arities:i.map(e=>t.get(e)??1),attrIs:a.map(e=>r.get(e)),attrIsSlots:new Map(a.map((e,t)=>[e,i.length+t]))}}var ut=new WeakMap,dt=new WeakMap,ft=new WeakMap;function pt(e){let t=``;return e.names.length>0&&(t+=`|params=[${e.names.map((t,n)=>`${JSON.stringify(t)}:${e.arities[n]}`).join(`,`)}]`),e.attrIs.length>0&&(t+=`|attrIs=[${e.attrIs.map(e=>X(e.attr,e.value)).join(`;`)}]`),t}function mt(e,t){let n=ct(e);if(n.names.length===0&&n.attrIs.length===0)return t;let r=ft.get(e);if(r!==void 0)return r;let i=`${ie(e).key}${pt(n)}`;return ft.set(e,i),i}function ht(e,t){return e.length===t.length&&e.every((e,n)=>Object.is(e,t[n]))}function gt(e,t){return t.constSlots===0?{values:[]}:t.attrIsSlots.length>0?{problem:`this kernel carries ${t.attrIsSlots.length} string-literal slot(s) (${t.attrIsSlots.map(e=>`${JSON.stringify(e.attr)} == ${JSON.stringify(e.value)}`).join(`, `)}) whose values are string-table indices of the geometry being cooked; fill them with constSlotValues, which takes that geometry's attribute set`}:_t(e,t)}function _t(e,t){let n=new Map,r;if(st(e,e=>{if(e.fn!==`param`||typeof e.name!=`string`||e.name===``)return;let t=e.name;if(h(e)!==void 0)return;let i=oe(e);if(i===void 0){r??=`param ${JSON.stringify(t)} has no bound value`;return}let a=typeof i==`number`?[i]:[...i],o=n.get(t);o===void 0?n.set(t,a):ht(o,a)||(r??=`param ${JSON.stringify(t)} is bound to two different values in one expression`)}),r!==void 0)return{problem:r};let i=[];for(let e of t.paramNames){let t=n.get(e);if(t===void 0)return{problem:`param ${JSON.stringify(e)} is not referenced by this spec`};for(let e=0;e<4;e++)i.push(e<t.length?t[e]:0)}return{values:i}}var vt=-1;function yt(e,t,n){if(t.constSlots===0)return{values:[]};let r=_t(e,t);if(`problem`in r||t.attrIsSlots.length===0)return r;let i=[...r.values];for(let e of t.attrIsSlots){let t=n.get(e.attr);if(t===void 0||t.type!==`string`)return{problem:`attributeIs ${JSON.stringify(e.attr)}: this geometry has no string attribute of that name (${t===void 0?`no such attribute`:`it is ${t.type}`}), so the literal has no index to resolve to`};let r=t.lookupString(e.value)??vt;for(let e=0;e<4;e++)i.push(e===0?r:0)}return{values:i}}var bt=new Set([`f32`,`i32`,`u32`,`bool`,`string`]);function xt(e){if(!z(e)||!z(e.attributes))throw new A(`compileFieldSpec: layout must be { attributes: { name: { type, tupleSize } } }`);for(let[t,n]of Object.entries(e.attributes)){if(!z(n)||!bt.has(n.type))throw new A(`kernel layout attribute ${JSON.stringify(t)}: unknown type ${JSON.stringify(n?.type)}; valid types: "f32", "i32", "u32", "bool", "string" (a string column binds as u32 and is readable only through attributeIs)`);let e=n.tupleSize;if(typeof e!=`number`||!Number.isInteger(e)||e<1)throw new A(`kernel layout attribute ${JSON.stringify(t)}: tupleSize must be a positive integer, got ${String(e)}`)}}function St(e){return typeof e==`number`?{fn:`constant`,value:e}:Array.isArray(e)?{fn:`constant`,value:[...e]}:e}function Ct(e){return e.type===`bool`||e.type===`string`?`u32`:e.type}function wt(e,t){xt(t);let n=St(e),r=ie(n),i=new Set;Y(n,i);let a=[...i].filter(e=>Object.hasOwn(t.attributes,e)).sort(),o=ct(n),s=new Fe(t,a,o),c=`f32`,l=0,u=[],d=e=>{if(l=e.size,e.size===1)u.push(`  outBuf[i] = ${e.ref};`);else for(let t=0;t<e.size;t++)u.push(`  outBuf[${W(e.size,t)}] = ${e.ref}.${Ne[t]};`)},f=n.fn===`attribute`?n.name:n.fn===`position`?`P`:void 0;if(n.fn===`index`)c=`u32`,l=1,u.push(`  outBuf[i] = i;`);else if(f!==void 0){let e=Re(s,`$`,f,n.fn===`position`?3:n.tupleSize,n.fn===`position`?`position reads `:``);if(e.type===`i32`||e.type===`u32`){c=e.type,l=e.tupleSize;let t=s.binding(f);for(let n=0;n<e.tupleSize;n++)u.push(`  outBuf[${W(e.tupleSize,n)}] = ${t.varName}[${W(e.tupleSize,n)}];`)}else d(Ve(n,`$`,s))}else d(Ve(n,`$`,s));let p=s.boundAttrs(),m=p.map(e=>({name:e.name,type:Ct(e.attr),tupleSize:e.attr.tupleSize,binding:e.binding})),h=p.length+1,g=[`@group(0) @binding(0) var<uniform> params: PcgParams;`];for(let e of p)g.push(`@group(0) @binding(${e.binding}) var<storage, read> ${e.varName}: array<${Ct(e.attr)}>; // attribute ${JSON.stringify(e.name)}: ${e.attr.type} tupleSize ${e.attr.tupleSize}`);g.push(`@group(0) @binding(${h}) var<storage, read_write> outBuf: array<${c}>;`);let _=o.names.length+o.attrIs.length,v=[`// Generated by pcg-ts compileFieldSpec (WGSL field kernel).
...
${[...s.lines,...u].join(`
`)}
}`],y=p.map(e=>`${JSON.stringify(e.name)}:${e.attr.type}x${e.attr.tupleSize}`).join(`,`),b=`${r.key}${pt(o)}`;return ft.set(n,b),{wgsl:`${v.join(`

`)}\n`,entryPoint:`main`,workgroupSize:je,outTupleSize:l,outType:c,inputs:m,bindings:{uniforms:0,output:h},constSlots:_,paramNames:o.names,attrIsSlots:o.attrIs,uniformBytes:_e(_),usesSeed:s.usesSeed,key:`${Me}|spec=${b}|layout=[${y}]`}}var Z={MAP_READ:1,COPY_SRC:4,COPY_DST:8,VERTEX:32,UNIFORM:64,STORAGE:128},Tt={READ:1},Et=256;function Dt(e){let t=Et;for(;t<e;)t*=2;return t}var Ot=class{device;maxPooledBytes;free=new Map;meta=new Map;detachedSet=new WeakSet;idleBytes=0;idleCount=0;created=0;reused=0;destroyed=0;detachedTotal=0;detachedLive=0;detachedLiveBytes=0;constructor(e,t){this.device=e,this.maxPooledBytes=t}acquire(e,t){let n=Dt(e),r=`${t}|${n}`,i=this.free.get(r)?.pop();if(i!==void 0)return this.idleBytes-=n,this.idleCount--,this.reused++,i;let a=this.device.createBuffer({size:n,usage:t});return this.meta.set(a,{key:r,bytes:n}),this.created++,a}release(e){let t=this.meta.get(e);if(t===void 0)throw this.detachedSet.has(e)?Error(`BufferPool.release: buffer was detached from this pool, so the pool no longer owns it and cannot reclaim it; destroy it through the DetachedBuffer that detach() returned (or the handle wrapping it) and stop releasing it`):Error(`BufferPool.release: buffer was not acquired from this pool`);if(this.idleBytes+t.bytes>this.maxPooledBytes){this.meta.delete(e),e.destroy(),this.destroyed++;return}let n=this.free.get(t.key);n===void 0&&(n=[],this.free.set(t.key,n)),n.push(e),this.idleBytes+=t.bytes,this.idleCount++}detach(e){let t=this.meta.get(e);if(t===void 0)throw Error(this.detachedSet.has(e)?`BufferPool.detach: buffer was already detached from this pool; ownership can only leave once — reuse the DetachedBuffer the first detach() returned`:`BufferPool.detach: buffer was not acquired from this pool`);this.meta.delete(e),this.detachedSet.add(e),this.detachedTotal++,this.detachedLive++,this.detachedLiveBytes+=t.bytes;let n=!1,r=this;return{buffer:e,bytes:t.bytes,get destroyed(){return n},destroy(){n||(n=!0,r.detachedLive--,r.detachedLiveBytes-=t.bytes,r.destroyed++,e.destroy())}}}get stats(){return{buffersCreated:this.created,buffersReused:this.reused,buffersDestroyed:this.destroyed,pooledBuffers:this.idleCount,pooledBytes:this.idleBytes,buffersDetached:this.detachedTotal,detachedBuffers:this.detachedLive,detachedBytes:this.detachedLiveBytes}}dispose(){for(let e of this.free.values())for(let t of e)this.meta.delete(t),t.destroy(),this.destroyed++;this.free.clear(),this.idleBytes=0,this.idleCount=0}},kt=`webgpu`,At=class{backend=kt;byteLength;detached;label;constructor(e,t,n){this.detached=e,this.byteLength=t,this.label=n}get disposed(){return this.detached.destroyed}get resource(){if(this.detached.destroyed)throw Error(`device transforms handle (${this.label}) was disposed; its GPU buffer is destroyed and cannot be bound. Dispose a handle only after the last frame that reads it, and re-cook to obtain a fresh one (device-resident outputs are never memoized, so every cook produces a new handle)`);return this.detached.buffer}dispose(){this.detached.destroy()}};function jt(e,t,n){return new At(e,t,n)}var Mt=65535;function Nt(e,t){let n=Mt*e;return Math.max(e,Math.floor(Math.min(t??n,n)/e)*e)}var Pt=16,Ft=`pcg-resident-run/5`;function It(e){return e.format===Ft?e:null}var Lt={reason:`run-plan-failed`},Rt=[`+x`,`-x`,`+y`,`-y`,`+z`,`-z`];function zt(e){if(typeof e!=`object`||!e||Array.isArray(e))return!1;let t=e;if(t.fn===`randomField`)return!0;for(let e of _(t))if(zt(e))return!0;return!1}function Bt(e){return Array.isArray(e)&&e.length===3&&e.every(e=>typeof e==`number`&&Number.isFinite(e))}var Q=class extends Error{};function Vt(e,t,n,r,i){let a=[...e].map(([e,t])=>({name:e,slot:t})),o=i||r===null,s=t.reduce((e,t)=>e+t.bytes,0),c=n.reduce((e,t)=>e+t,0),l=o?a.reduce((e,n)=>e+t[n.slot].bytes,0):0;return{writtenList:a,materialize:o,totalBytes:s+c+l+(r?.bytes??0)+(r?.colorBytes??0)+(r?.permBytes??0)}}function Ht(e,t,n,r){let i=t.count,a=new Map(Object.entries(t.attributes)),o=[],c=new Map,l=[],u=new Map,d=[],f=[],p=null,m=()=>Object.fromEntries(a),h=e=>{let t=c.get(e);if(t!==void 0)return t;let n=a.get(e);if(n===void 0||n.type===`string`)throw new Q(e);let r=o.length;return o.push({bytes:i*n.tupleSize*4,init:`attr`,name:e}),c.set(e,r),r},g=(e,t,n)=>{let r=o.length;return o.push({bytes:i*t*4,init:n,name:e}),c.set(e,r),r},_=(e,t,n)=>{let r=a.get(e);if(r===void 0||r.type!==t||r.tupleSize!==n)throw new Q(e)},v=(e,t,n)=>{let r=t.length/4;if(r>=4)throw Error(`resident run: "${n}" needs more than 4 uniform constant slots for its constant params; raise MAX_APPLY_CONST_SLOTS in applyKernels.ts (each slot costs 16 bytes of the per-chunk uniform and nothing else)`);for(let n=0;n<4;n++)t.push(n<e.length?e[n]:0);return{kind:`const`,tupleSize:e.length,slot:r}},y=(e,t,n,a,o,s)=>{let c;if(C(e)){let t=O(e,r);if(t===void 0)throw new Q(`no spec`);if(u.has(`P`)&&zt(t))throw new Q(`identity after P write`);c=t}else if(typeof e==`number`||Array.isArray(e)&&e.every(e=>typeof e==`number`)){let t=typeof e==`number`?[e]:e;if(t.length<1||t.length>4||a!==null&&!a.includes(t.length))throw new Q(`tuple`);for(let e of t)if(!Number.isFinite(Math.fround(e)))throw new Q(`f32 range`);return{param:v(t,o,s),ref:null}}else throw new Q(`bad param value`);let d;try{d=wt(c,{attributes:m()})}catch{throw new Q(`compile`)}if(d.inputs.length+1>8)throw new Q(`buffers`);if(a!==null&&!a.includes(d.outTupleSize))throw new Q(`tuple`);if(d.attrIsSlots.length>0)throw new Q(`attributeIs / byAttribute need a per-dispatch string table`);let f=gt(c,d);if(`problem`in f)throw new Q(`param bindings`);let p=l.length;return l.push(i*d.outTupleSize*4),n.push({key:d.key,wgsl:d.wgsl,entryPoint:d.entryPoint,workgroupSize:d.workgroupSize,seed:t,uniformsBinding:d.bindings.uniforms,uniformBytes:d.uniformBytes,consts:f.values,perBatch:!1,bindings:[...d.inputs.map(e=>({binding:e.binding,ref:{kind:`slot`,index:h(e.name)}})),{binding:d.bindings.output,ref:{kind:`col`,index:p}}]}),{param:{kind:`column`,type:d.outType,tupleSize:d.outTupleSize},ref:{kind:`col`,index:p}}},b=(e,t,n,r,i=!1)=>{if(e.constSlots*4!==r.length)throw Error(`resident run: apply kernel "${e.key}" declares ${e.constSlots} constant slots but the planner allocated ${r.length/4}`);return{key:e.key,wgsl:e.wgsl,entryPoint:e.entryPoint,workgroupSize:e.workgroupSize,seed:t,uniformsBinding:0,uniformBytes:e.uniformBytes,consts:r,perBatch:i,bindings:e.bindings.map(e=>{let t=n[e.role];if(t===void 0)throw new Q(`unmapped role ${e.role}`);return{binding:e.binding,ref:t}})}};try{for(let t of e){let n=t===e[e.length-1],r=[],o=[],c=t.params;switch(t.kind){case`setAttribute`:{let e=c.name,n=c.type,i=c.tupleSize;if(typeof e!=`string`)throw new Q(`name`);if(n!==`f32`&&n!==`i32`&&n!==`u32`&&n!==`bool`)throw new Q(`type`);if(typeof i!=`number`||!Number.isInteger(i)||i<1||i>4)throw new Q(`tupleSize`);let l=typeof c.seed==`number`?c.seed:NaN,f=l===0?t.seed:s(t.seed,l),{param:p,ref:m}=y(c.value,f,r,i===1?[1]:[1,i],o,t.kind),h=g(e,i,`none`);a.set(e,{type:n,tupleSize:i}),u.set(e,h),d.push({op:`replace`,name:e,type:n,tupleSize:i});let _={target:{kind:`slot`,index:h}};m!==null&&(_.value=m),r.push(b(we(p,n,i),0,_,o));break}case`transformPoints`:{_(`P`,`f32`,3);let e=y(c.translate,t.seed,r,[1,3],o,t.kind),n=y(c.rotateEuler,t.seed,r,[1,3],o,t.kind),i=y(c.scale,t.seed,r,[1,3],o,t.kind),s=a.get(`rot`),l=s!==void 0&&s.type===`f32`&&s.tupleSize===4,d=a.get(`scale`),f=d!==void 0&&d.type===`f32`&&d.tupleSize===3,p=h(`P`);u.set(`P`,p);let m={P:{kind:`slot`,index:p}};if(e.ref!==null&&(m.translate=e.ref),n.ref!==null&&(m.rotateEuler=n.ref),i.ref!==null&&(m.scale=i.ref),l){let e=h(`rot`);u.set(`rot`,e),m.rot={kind:`slot`,index:e}}if(f){let e=h(`scale`);u.set(`scale`,e),m.scaleAttr={kind:`slot`,index:e}}r.push(b(Ee(e.param,n.param,i.param,l,f),0,m,o));break}case`jitterPoints`:{if(_(`P`,`f32`,3),u.has(`P`))throw new Q(`identity after P write`);let e=typeof c.seed==`number`?c.seed:NaN,n=s(t.seed,e),i=y(c.amount,n,r,[1,3],o,t.kind),l=a.get(`seed`),d=l!==void 0;if(d&&(l.type!==`u32`||l.tupleSize!==1))throw new Q(`seed attribute shape`);let f=h(`P`);u.set(`P`,f);let p={P:{kind:`slot`,index:f}};i.ref!==null&&(p.amount=i.ref),d&&(p.seed={kind:`slot`,index:h(`seed`)}),r.push(b(De(i.param,d),n,p,o));break}case`orientAlongVector`:{let e=c.axis;if(!Rt.includes(e))throw new Q(`axis`);if(!Bt(c.up))throw new Q(`up`);let n=y(c.direction,t.seed,r,[1,3],o,t.kind),i=c.up,s=i[0]*i[0]+i[1]*i[1]+i[2]*i[2],l=s>0?1/Math.sqrt(s):0,f=[i[0]*l,i[1]*l,i[2]*l];for(let e of f)if(!Number.isFinite(Math.fround(e)))throw new Q(`up range`);let p=v(f,o,t.kind),m=a.get(`rot`),_=m!==void 0&&m.type===`f32`&&m.tupleSize===4?h(`rot`):g(`rot`,4,`quat-default`);a.set(`rot`,{type:`f32`,tupleSize:4}),u.set(`rot`,_),d.push({op:`ensure-rot`});let x={rot:{kind:`slot`,index:_}};n.ref!==null&&(x.direction=n.ref),r.push(b(ke(n.param,e,p),0,x,o));break}case`spawnInstances`:{if(!n)throw new Q(`spawnInstances must be the run's last member`);let e=c.assetId;if(typeof e!=`string`||e===``)throw new Q(`assetId`);if(_(`P`,`f32`,3),i>1048576)throw new Q(`${i} instances over MAX_INSTANCES`);let t=c.assetAttr;if(t!==void 0&&typeof t!=`string`)throw new Q(`assetAttr`);let s=t===void 0?``:t;if(s!==``){let e=a.get(s);if(e===void 0)throw new Q(`assetAttr "${s}" not on the point domain`);if(e.type!==`string`)throw new Q(`assetAttr "${s}" is ${e.type}, not string`)}let l=c.colorAttr;if(l!==void 0&&typeof l!=`string`)throw new Q(`colorAttr`);let u=l===void 0?``:l,d=0;if(u!==``){let e=a.get(u);if(e===void 0)throw new Q(`colorAttr "${u}" not on the point domain`);if(e.type!==`f32`||e.tupleSize<3)throw new Q(`colorAttr "${u}" is ${e.type}x${e.tupleSize}`);d=e.tupleSize}let f=a.get(`rot`),m=f!==void 0&&f.type===`f32`&&f.tupleSize===4,g=a.get(`scale`),v=g!==void 0&&g.type===`f32`&&g.tupleSize===3,y={P:{kind:`slot`,index:h(`P`)},transforms:{kind:`out`}};m&&(y.rot={kind:`slot`,index:h(`rot`)}),v&&(y.scaleAttr={kind:`slot`,index:h(`scale`)});let x=s!==``;x&&(y.perm={kind:`perm`}),d>0&&(y.color={kind:`slot`,index:h(u)},y.colors={kind:`colorOut`}),r.push(b(Ae(m,v,x,d),0,y,o,x)),p={assetId:e,assetAttr:s,colorAttr:u,colorTupleSize:d,count:i,bytes:i*64,colorBytes:d>0?i*Pt:0,permBytes:x?i*4:0};break}default:throw new Q(`unknown kind ${t.kind}`)}f.push({id:t.id,type:t.type,steps:r})}}catch(e){if(e instanceof Q)return Lt;throw e}let{writtenList:x,materialize:S,totalBytes:w}=Vt(u,o,l,p,t.needsGeometry);return w>n?{reason:`run-too-large`}:{plan:{format:Ft,count:i,members:f,slots:o,cols:l,written:x,layoutOps:d,materialize:S,instances:p,totalBytes:w}}}var Ut={f32:Float32Array,i32:Int32Array,u32:Uint32Array};function Wt(){return new Promise(e=>setTimeout(e,0))}async function Gt(e,t,n,r){let{device:i,pool:a}=e,{geo:o,signal:s,budgetMs:c}=n,l=t.count;if(o.attrs.point.count!==l)throw Error(`resident run: plan was built for ${l} points but the input geometry has ${o.attrs.point.count}; plans are single-cook artifacts — re-plan for new inputs`);let u=()=>{if(s?.aborted)throw new w},d=[],f=(e,t)=>{let n=a.acquire(e,t);return d.push(n),n},p=new Set,m=[];try{let n=o.attrs.point,s=t.slots.map(e=>{let t=f(e.bytes,Z.STORAGE|Z.COPY_DST|Z.COPY_SRC);if(e.init===`attr`){let r=n.require(e.name),a=e.bytes/4;if(r.data instanceof Uint8Array){let e=new Uint32Array(a);for(let t=0;t<a;t++)e[t]=r.data[t];i.queue.writeBuffer(t,0,e)}else i.queue.writeBuffer(t,0,r.data.subarray(0,a))}else if(e.init===`quat-default`){let n=new Float32Array(e.bytes/4);for(let e=3;e<n.length;e+=4)n[e]=1;i.queue.writeBuffer(t,0,n)}return t}),d=t.cols.map(e=>f(e,Z.STORAGE|Z.COPY_DST|Z.COPY_SRC)),h=t.instances===null?void 0:ee(o,{defaultAssetId:t.instances.assetId,...t.instances.assetAttr===``?{}:{assetAttr:t.instances.assetAttr}}),g=t.instances!==null&&t.instances.permBytes>0?f(t.instances.permBytes,Z.STORAGE|Z.COPY_DST):void 0;g!==void 0&&h!==void 0&&i.queue.writeBuffer(g,0,h.perm);let _=Z.STORAGE|Z.COPY_DST|Z.COPY_SRC|Z.VERTEX,v=h===void 0?[]:Array.from(h.counts,e=>f(e*64,_)),y=h===void 0||t.instances===null||t.instances.colorBytes===0?[]:Array.from(h.counts,e=>f(e*Pt,_)),b=(e,t)=>{if(e.kind===`slot`)return s[e.index];if(e.kind===`col`)return d[e.index];if(e.kind===`colorOut`){let e=y[t];if(e===void 0)throw Error(`resident run: a kernel binds a retained instance-colour buffer but the plan declares no colour output (plan and kernels disagree)`);return e}if(e.kind===`perm`){if(g===void 0)throw Error(`resident run: a kernel binds the grouping permutation but the plan declares no per-point asset attribute (plan and kernels disagree)`);return g}let n=v[t];if(n===void 0)throw Error(`resident run: a kernel binds a retained instance-transform buffer but the plan declares no instances output (plan and kernels disagree)`);return n},x=i.createCommandEncoder(),S=x.beginComputePass(),C=performance.now();for(let n of t.members){u();for(let t of n.steps){let n=e.getPipeline(t.key,t.wgsl,t.entryPoint,r);S.setPipeline(n);let a=Nt(t.workgroupSize,e.maxElementsPerDispatch),o=t.perBatch&&h!==void 0?Array.from(h.counts,(e,t)=>({batch:t,elements:e,base:h.offsets[t]})):[{batch:0,elements:l,base:0}];for(let e of o){r!==void 0&&r.dispatches++;let o=new ArrayBuffer(t.uniformBytes),s=new Uint8Array(o),c=new Uint32Array(o,0,t.uniformBytes>=16?4:3);c[0]=e.elements,c[1]=t.seed>>>0,t.perBatch&&(c[3]=e.base),t.consts.length>0&&new Float32Array(o,16,t.consts.length).set(t.consts);let l=Math.ceil(e.elements/a);for(let r=0;r<l;r++){let o=f(t.uniformBytes,Z.UNIFORM|Z.COPY_DST);c[2]=r*a,i.queue.writeBuffer(o,0,s);let l=i.createBindGroup({layout:n.getBindGroupLayout(0),entries:[{binding:t.uniformsBinding,resource:{buffer:o}},...t.bindings.map(t=>({binding:t.binding,resource:{buffer:b(t.ref,e.batch)}}))]}),u=Math.min(a,e.elements-r*a);S.setBindGroup(0,l),S.dispatchWorkgroups(Math.ceil(u/t.workgroupSize))}}}c!==void 0&&performance.now()-C>c&&(await Wt(),u(),C=performance.now())}S.end();let w=[],T,E=t.materialize?t.written.reduce((e,n)=>e+t.slots[n.slot].bytes,0):0;if(E>0){T=f(E,Z.COPY_DST|Z.MAP_READ);let e=0;for(let n of t.written){let r=t.slots[n.slot].bytes;x.copyBufferToBuffer(s[n.slot],0,T,e,r),w.push(e),e+=r}}i.queue.submit([x.finish()]);let D;if(t.materialize){let e;if(T!==void 0){await T.mapAsync(Tt.READ,0,E);try{e=T.getMappedRange(0,E).slice(0)}finally{T.unmap()}}u(),D=re(o);let n=D.attrs.point;for(let e of t.layoutOps)if(e.op===`replace`)n.replace(e.name,e.type,e.tupleSize);else{let e=n.get(`rot`);(!e||e.type!==`f32`||e.tupleSize!==4)&&(e&&n.remove(`rot`),n.add(`rot`,`f32`,4,[0,0,0,1]))}t.written.forEach((t,r)=>{let i=n.require(t.name),a=l*i.tupleSize;if(e===void 0)throw Error(`resident run: readback missing for a written attribute`);if(i.data instanceof Uint8Array){let t=new Uint32Array(e,w[r],a);for(let e=0;e<a;e++)i.data[e]=t[e]}else{let n=Ut[i.type];if(n===void 0)throw Error(`resident run: cannot materialize attribute "${t.name}" of type ${i.type}`);i.data.set(new n(e,w[r],a))}})}else u();let O;if(t.instances!==null){let e=t.instances.colorBytes>0;if(h===void 0||v.length!==h.order.length||y.length!==(e?h.order.length:0))throw Error(`resident run: the plan declares an instances output but the acquired transform buffers do not match the grouping (library bug: plan.instances, the grouping, and the acquired buffers must agree)`);let n=(e,t,n)=>{let r=a.detach(e);p.add(e);try{return jt(r,t,n)}catch(e){throw r.destroy(),e}},r=[];for(let t=0;t<h.order.length;t++){let i=h.order[t],a=h.counts[t],o=n(v[t],a*64,`${a} instances of "${i}"`);if(m.push(o),!e){r.push({residency:`device`,assetId:i,count:a,transforms:o});continue}let s=n(y[t],a*Pt,`${a} instance colours of "${i}"`);m.push(s),r.push({residency:`device`,assetId:i,count:a,transforms:o,colors:s})}O=r}r!==void 0&&(r.residentRuns++,r.fusedNodes+=t.members.length,r.readbacksSaved+=t.members.length-+!!t.materialize);let te={};return D!==void 0&&(te.geo=D),O!==void 0&&(te.deviceBatches=O),te}catch(e){for(let e of m)e.dispose();throw e instanceof w?e:Error(`GpuFieldEvaluator: resident run failed (${t.members.length} fused nodes [${t.members.map(e=>`"${e.id}"`).join(`, `)}], ${l} points): ${e instanceof Error?e.message:String(e)}`,{cause:e})}finally{for(let e of d)p.has(e)||a.release(e)}}var Kt=`gpu2`,qt=268435456,Jt=[`spawnInstances`],Yt={f32:Float32Array,i32:Int32Array,u32:Uint32Array},Xt={f32:Float32Array,i32:Int32Array,u32:Uint32Array};function Zt(e){let t=e=>e!==void 0&&e!==``?e:`?`;return[Kt,t(e?.vendor),t(e?.architecture),t(e?.device),t(e?.description)].join(`|`)}function $(e,t){return e!==void 0&&(e.fallbacks[t]=(e.fallbacks[t]??0)+1),null}var Qt=class{cacheSalt;residentTerminals;acceptDerivedSpecs;device;kernels=new Map;pipelines=new Map;pool;maxElementsPerDispatch;maxResidentBytes;constructor(e,t={}){if(t.maxElementsPerDispatch!==void 0&&!Number.isFinite(t.maxElementsPerDispatch))throw Error(`GpuFieldEvaluator: maxElementsPerDispatch must be a finite number, got ${t.maxElementsPerDispatch}; leave it unset to use the device maximum`);this.device=e,this.cacheSalt=Zt(t.adapterInfo??e.adapterInfo),this.pool=new Ot(e,t.maxPooledBytes??qt),this.maxElementsPerDispatch=t.maxElementsPerDispatch,this.maxResidentBytes=t.maxResidentBytes??536870912,this.residentTerminals=t.deviceInstances===!0?Jt:[],this.acceptDerivedSpecs=ne(t)}get pipelineCacheSize(){return this.pipelines.size}get kernelCacheSize(){return this.kernels.size}get poolStats(){return this.pool.stats}dispose(){this.pool.dispose()}chunkElements(e){let t=Mt*e.workgroupSize,n=Math.min(this.maxElementsPerDispatch??t,t);return Math.max(e.workgroupSize,Math.floor(n/e.workgroupSize)*e.workgroupSize)}resolveField(e,t,n){let r=O(e,this.acceptDerivedSpecs);if(r===void 0)return $(n,le(e));let i=t.geo.attrs[t.domain],a={},o=[];for(let e of i.names().sort()){let t=i.get(e);t!==void 0&&(a[e]={type:t.type,tupleSize:t.tupleSize},o.push(`${JSON.stringify(e)}:${t.type}x${t.tupleSize}`))}let s;try{s=mt(r,e.key)}catch{return $(n,`compile-error`)}let c=`${s.length}#${s}|${o.join(`,`)}`,l=this.kernels.get(c);if(l===void 0){try{l=wt(r,{attributes:a})}catch(e){l=e instanceof Error?e:Error(String(e))}this.kernels.set(c,l)}if(l instanceof Error)return $(n,`compile-error`);if(l.inputs.length+1>8)return $(n,`too-many-buffers`);let u=yt(r,l,i);if(`problem`in u)return $(n,`param-bindings`);let d=i.count;if(d===0)return Promise.resolve({data:new Xt[l.outType](0),tupleSize:l.outTupleSize});let f=this.getPipeline(l.key,l.wgsl,l.entryPoint,n);return n!==void 0&&n.dispatches++,this.dispatch(e,t,l,f,d,u.values)}getPipeline(e,t,n,r){let i=this.pipelines.get(e);if(i!==void 0)return r!==void 0&&r.pipelineCacheHits++,i;let a=this.device.createShaderModule({code:t}),o=this.device.createComputePipeline({layout:`auto`,compute:{module:a,entryPoint:n}});return this.pipelines.set(e,o),r!==void 0&&r.pipelinesCompiled++,o}planRun(e,t,n){let r=Ht(e,t,this.maxResidentBytes,this.acceptDerivedSpecs);return`plan`in r?r.plan:(n!==void 0&&(n.fallbacks[r.reason]=(n.fallbacks[r.reason]??0)+1),null)}executeRun(e,t,n){let r=It(e);return r===null?Promise.reject(Error(`GpuFieldEvaluator.executeRun: plan was not produced by this library's planRun; pass the object returned by planRun on the same resolver`)):Gt({device:this.device,pool:this.pool,maxElementsPerDispatch:this.maxElementsPerDispatch,getPipeline:(e,t,n,r)=>this.getPipeline(e,t,n,r)},r,t,n)}async dispatch(e,t,n,r,i,a){let o=this.device,s=[],c=(e,t)=>{let n=this.pool.acquire(e,t);return s.push(n),n};try{let e=this.chunkElements(n),s=Math.ceil(i/e),l=[],u=t.geo.attrs[t.domain];for(let e of n.inputs){let t=u.require(e.name),n=i*e.tupleSize,r;if(t.data instanceof Uint8Array){let e=new Uint32Array(n);for(let r=0;r<n;r++)e[r]=t.data[r];r=e}else r=t.data.subarray(0,n);let a=c(n*4,Z.STORAGE|Z.COPY_DST);o.queue.writeBuffer(a,0,r),l.push({binding:e.binding,resource:{buffer:a}})}let d=i*n.outTupleSize*4,f=c(d,Z.STORAGE|Z.COPY_SRC);l.push({binding:n.bindings.output,resource:{buffer:f}});let p=c(d,Z.COPY_DST|Z.MAP_READ),m=new ArrayBuffer(n.uniformBytes),h=new Uint8Array(m),g=new Uint32Array(m,0,3);g[0]=i,g[1]=t.seed>>>0,a.length>0&&new Float32Array(m,16,a.length).set(a);let _=[];for(let t=0;t<s;t++){let i=c(n.uniformBytes,Z.UNIFORM|Z.COPY_DST);g[2]=t*e,o.queue.writeBuffer(i,0,h),_.push(o.createBindGroup({layout:r.getBindGroupLayout(0),entries:[{binding:n.bindings.uniforms,resource:{buffer:i}},...l]}))}let v=o.createCommandEncoder(),y=v.beginComputePass();y.setPipeline(r);for(let t=0;t<s;t++){let r=Math.min(e,i-t*e);y.setBindGroup(0,_[t]),y.dispatchWorkgroups(Math.ceil(r/n.workgroupSize))}y.end(),v.copyBufferToBuffer(f,0,p,0,d),o.queue.submit([v.finish()]),await p.mapAsync(Tt.READ,0,d);let b;try{b=p.getMappedRange(0,d).slice(0)}finally{p.unmap()}return{data:new Yt[n.outType](b),tupleSize:n.outTupleSize}}catch(n){throw Error(`GpuFieldEvaluator: dispatch failed for field ${e.key} (${i} elements on the ${t.domain} domain): ${n instanceof Error?n.message:String(n)}`,{cause:n})}finally{for(let e of s)this.pool.release(e)}}};function $t(e){return Array.isArray(e)?e.map(e=>e.clone()):e.clone()}function en(e){return Array.isArray(e)?e:[e]}function tn(e,t){let n=[];try{for(let i of e){let e=t[i.assetId];if(!e){let e=Object.keys(t).sort().join(`, `);throw Error(`toInstancedMeshes: unknown assetId "${i.assetId}"; known assets: `+(e===``?`(none)`:e))}if(i.transforms.length!==i.count*16)throw Error(`toInstancedMeshes: batch "${i.assetId}" has ${i.transforms.length} transform floats, expected count * 16 = ${i.count*16}`);if(i.colors&&i.colors.length!==i.count*3)throw Error(`toInstancedMeshes: batch "${i.assetId}" has ${i.colors.length} colour floats, expected count * 3 = ${i.count*3} (rgb per instance; alpha is dropped at the spawner)`);let a=new f(e.geometry,$t(e.material),i.count);a.instanceMatrix.array.set(i.transforms),a.instanceMatrix.needsUpdate=!0,i.colors&&(a.instanceColor=new r(i.colors.slice(),3),a.instanceColor.needsUpdate=!0),a.name=i.assetId,a.computeBoundingSphere(),n.push(a)}}catch(e){for(let e of n){e.dispose();for(let t of en(e.material))t.dispose()}throw e}return n}function nn(e,t={}){let n=e.attrs.point,r=n.get(`P`);if(!r||r.type!==`f32`||r.tupleSize!==3)throw Error(`toPointsObject: geometry needs a point attribute "P" (f32, tupleSize 3)`);let i=n.count,a=new c;a.setAttribute(`position`,new o(r.data.slice(0,i*3),3));let s=!1,l=t.useColor===!1?void 0:x(n.get(`color`));if(l){let e=new Float32Array(i*3);for(let t=0;t<i;t++)E(e,t*3,l,t);a.setAttribute(`color`,new o(e,3)),s=!0}let u=new p({size:t.size??.1,sizeAttenuation:!0,vertexColors:s});return new v(a,u)}export{Qt as a,tn as i,$t as n,en as r,nn as t};