import{Ai as e,Di as t,Ei as n,Gr as r,Gt as i,Ii as a,Kr as o,Li as s,Ni as c,Oi as l,Pi as u,Q as d,Ri as f,Sa as p,Wr as m,Wt as h,_a as g,aa as _,ba as v,bi as y,ca as b,da as x,et as S,fa as C,ga as w,ha as T,ia as E,ji as D,ki as O,l as k,la as A,ma as ee,pa as te,sa as ne,u as re,ua as ie,va as ae,ya as oe,zi as se}from"./mobile-QNTV8uaS.js";var j=class extends Error{constructor(e){super(e),this.name=`GpuCompileError`}};function M(e,t){let n=Math.fround(e);if(!Number.isFinite(n))throw new j(`${t}: value ${e} is not representable as a finite f32 (WGSL kernels compute in f32; keep magnitudes within ~3.4e38)`);return Object.is(n,-0)?`-0f`:`${String(n)}f`}function ce(e){return`${e>>>0}u`}function N(e){return`0x${(e>>>0).toString(16).padStart(8,`0`)}u`}var P=N,le=M(34028234663852886e22,`internal f32 max`);function ue(e,t){let n=P(e);for(let e of t)n=`pcg_hash_mix(${n}, ${e})`;return`pcg_hash_finalize(${n})`}function de(){let e=[];for(let t=0;t<12;t++){let n=e=>M(a[t*3+e],`internal GRAD3`);e.push(`  vec3<f32>(${n(0)}, ${n(1)}, ${n(2)}),`)}return`var<private> PCG_GRAD3: array<vec3<f32>, 12> = array<vec3<f32>, 12>(
${e.join(`
`)}
  ...
  return ${F(`internal PERLIN_SCALE`)(u)} * pcg_mix(
  ...
  let t = ${F(`internal simplex R2`)(e)} - x * x - y * y - z * z;
  if (t <= 0f) {
    return 0f;
  }
  ...
  let s = (p.x + p.y + p.z) * ${F(`internal simplex F3`)(l)};
  let i = i32(floor(p.x + s));
  let j = i32(floor(p.y + s));
  let k = i32(floor(p.z + s));
  let t = f32(i + j + k) * ${F(`internal simplex G3`)(O)};
  let x0 = p.x - (f32(i) - t);
  let y0 = p.y - (f32(j) - t);
  let z0 = p.z - (f32(k) - t);
  var i1: i32;
  ...
  let x1 = x0 - f32(i1) + ${F(`internal simplex G3`)(O)};
  let y1 = y0 - f32(j1) + ${F(`internal simplex G3`)(O)};
  let z1 = z0 - f32(k1) + ${F(`internal simplex G3`)(O)};
  let x2 = x0 - f32(i2) + ${F(`internal simplex 2*G3`)(2*O)};
  let y2 = y0 - f32(j2) + ${F(`internal simplex 2*G3`)(2*O)};
  let z2 = z0 - f32(k2) + ${F(`internal simplex 2*G3`)(2*O)};
  let x3 = x0 - 1f + ${F(`internal simplex 3*G3`)(3*O)};
  let y3 = y0 - 1f + ${F(`internal simplex 3*G3`)(3*O)};
  let z3 = z0 - 1f + ${F(`internal simplex 3*G3`)(3*O)};
