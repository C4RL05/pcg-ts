import{$i as e,Da as t,Ei as n,G as r,Hr as i,K as a,N as o,Ni as ee,Nt as s,Ui as te,Ur as ne,Ut as c,Xi as l,Y as u,Yi as d,_i as re,ca as ie,ci as ae,da as oe,gi as se,ia as ce,la as le,oa as f,pi as ue,r as p,si as de,vi as fe,wt as m}from"./mobile-BNVzpA-x.js";import{t as h}from"./scene-1yzHrewc.js";import{a as g}from"./debug-CSycFCoi.js";import{t as pe}from"./worldBinding-DJlq3brF.js";import{t as me}from"./overlay-BvBZFCsp.js";var he=.35,ge=777;function _e(){let e=new n,r=e.add(de,{count:170,boundsMin:[-800,0,-800],boundsMax:[800,0,800]}),i=e.add(fe,{name:`scale`,tupleSize:3,value:(()=>{let e=f(oe(`mega`),0,1,5,13);return ie(e,ce(e,1.6),e)})()}),a=e.add(ne,{assetId:`megarock`});return e.connect(r,`out`,i,`in`),e.connect(i,`out`,a,`in`),e.output(a,`instances`,`instances`),{name:`landmarks`,cellSize:`unbounded`,generationRadius:1/0,graph:e,bind(e,n){e.setParam(r,`seed`,n.seed),e.setParam(i,`seed`,t(n.seed,9))}}}function ve(r){let{cellSize:i,generationRadius:a,anchored:o}=r,s=r.halo?4:0,c=new n,l=o?c.add(ae,{density:he,cellSize:7,latticeMode:`xz`,height:0}):c.add(de,{count:Math.round(he*(i+2*s)**2)}),u=c.add(fe,{name:`density`,tupleSize:1,value:f(ee(te,{seed:ge,frequency:.02,octaves:3}),-1,1,0,1)}),d=c.add(re,{mode:`probabilistic`}),p=c.add(ue,{radius:4,countAttr:`nbrCount`}),m=c.add(se,{mode:`inside`}),h=c.add(fe,{name:`scale`,tupleSize:3,value:(()=>{let t=f(e(le(`nbrCount`),2,16),2,16,.4,1.35),n=ce(t,f(oe(`rock`),0,1,.85,1.15));return ie(n,n,n)})()}),g=c.add(ne,{assetId:`rock`});return c.connect(l,`out`,u,`in`),c.connect(u,`out`,d,`in`),c.connect(d,`out`,p,`in`),c.connect(p,`out`,m,`in`),c.connect(m,`out`,h,`in`),c.connect(h,`out`,g,`in`),c.output(g,`instances`,`instances`),{name:`rocks`,cellSize:i,generationRadius:a,graph:c,bind(e,n){e.setParam(l,`boundsMin`,[n.min[0]-s,0,n.min[1]-s]),e.setParam(l,`boundsMax`,[n.max[0]+s,0,n.max[1]+s]);let r=o?n.worldSeed:n.seed;e.setParam(l,`seed`,r),e.setParam(d,`seed`,t(r,1)),e.setParam(h,`seed`,t(r,2)),e.setParam(m,`boundsMin`,[n.min[0],-1,n.min[1]]),e.setParam(m,`boundsMax`,[n.max[0],1,n.max[1]])}}}function ye(e){return e instanceof Error?e.message:String(e)}function be(e){let t=(e?.description??``)===``?e?.device:e?.description;return[e?.vendor,e?.architecture,t].filter(e=>typeof e==`string`&&e!==``).join(` · `)||`adapter (no info exposed)`}async function xe(e){let t=navigator.gpu;if(t===void 0)return{error:`navigator.gpu is missing — this browser has no WebGPU.`};try{let n=await t.requestAdapter();if(n===null)return{error:`navigator.gpu.requestAdapter() returned null — no compatible GPU adapter.`};let r=n.info,i=await n.requestDevice(),a=i.lost;return a!==void 0&&a.then(t=>{e(`${t?.reason??`unknown`}: ${t?.message??`no detail`}`)}),{device:i,info:r,label:be(r)}}catch(e){return{error:`requestDevice() failed: ${ye(e)}`}}}function Se(e){let t=e.info===void 0?{}:{adapterInfo:e.info};return{derived:new g(e.device,{...t,acceptDerivedSpecs:!0}),strict:new g(e.device,t)}}var Ce=class{cacheSalt;residentTerminals;acceptDerivedSpecs;base;seen=new Set;constructor(e){this.base=e,this.cacheSalt=e.cacheSalt,this.residentTerminals=e.residentTerminals,this.acceptDerivedSpecs=e.acceptDerivedSpecs}resolveField(e,t,n){return n!==void 0&&this.seen.add(n),this.base.resolveField(e,t,n)}planRun(e,t,n){return n!==void 0&&this.seen.add(n),this.base.planRun(e,t,n)}executeRun(e,t,n){return n!==void 0&&this.seen.add(n),this.base.executeRun(e,t,n)}drain(){if(this.seen.size===0)return;let e=l();for(let t of this.seen){e.dispatches+=t.dispatches,e.pipelinesCompiled+=t.pipelinesCompiled,e.pipelineCacheHits+=t.pipelineCacheHits,e.residentRuns+=t.residentRuns,e.fusedNodes+=t.fusedNodes,e.readbacksSaved+=t.readbacksSaved;for(let[n,r]of Object.entries(t.fallbacks))e.fallbacks[n]=(e.fallbacks[n]??0)+r}return this.seen.clear(),e}};function we(e){let t=Object.entries(e).sort(([e],[t])=>e.localeCompare(t));return t.length===0?`none`:t.map(([e,t])=>`${e} ×${t}`).join(`, `)}var _=1,v=18,y=140,b=`gpu`,x=!0,S=20,C=!0,Te=!0,Ee=!0,De=7,Oe=12,ke=8,Ae=5e3;function je(e){return e instanceof Error?e.message:String(e)}var{scene:w,camera:T,start:Me}=h({cameraPosition:[0,26,0],orbit:!1,fog:{near:140,far:640},far:1600}),E=new m(new c(4e3,4e3),new s({color:1449515,roughness:1}));E.rotation.x=-Math.PI/2,E.position.y=-.03,w.add(E);var Ne=40,D;function Pe(){D&&(w.remove(D),D.geometry.dispose(),D.material.dispose());let e=new r(S*Ne,Ne,4158656,2903936),t=e.material;t.transparent=!0,t.opacity=.45,t.depthWrite=!1,e.position.y=.02,e.visible=Ee,w.add(e),D=e}function Fe(){!D||!D.visible||(D.position.x=Math.round(T.position.x/S)*S,D.position.z=Math.round(T.position.z/S)*S)}var Ie=new s({color:8028816,roughness:.95,flatShading:!0}),Le=new s({color:6121077,roughness:1,flatShading:!0}),Re={megarock:{geometry:new o(1).translate(0,.62,0),material:Le},rock:{geometry:new u(.55).translate(0,.33,0),material:Ie}},ze,Be,O=`requesting…`,k,A=!1,Ve=``,j;function M(){return ze!==void 0&&!A}function He(){if(!(b===`cpu`||!M()))return x?ze:Be}function Ue(e,t){let n=e*1.25/t+1.5;return Math.max(256,Math.ceil(Math.PI*n*n))}function We(e){let t=0;for(let n of Object.keys(e))for(let r of e[n])if(r.kind===`instances`)for(let e of r.batches)t+=e.count;return t}var N,P=0,F,I=0,L=!1,Ge=0;function R(){N&&(N.disposed=!0,N.abort.abort(),N.binding.dispose(),w.remove(N.group));let e=new a;w.add(e);let t=new pe({group:e,assets:Re}),n=new Map,r=He(),o={world:void 0,group:e,binding:t,cellInstances:n,tap:r===void 0?void 0:new Ce(r),abort:new AbortController,disposed:!1};o.world=new i({seed:_,levels:[_e(),ve({cellSize:S,generationRadius:y,anchored:C,halo:Te})],maxCellsPerLevel:Ue(y,S),...o.tap===void 0?{}:{gpu:o.tap},onCellReady:(e,r,i)=>{o.disposed||(t.cellReady(e,r,i),n.set(`${e}|${r[0]},${r[1]}`,We(i)))},onCellEvicted:(e,r)=>{o.disposed||(t.cellEvicted(e,r),n.delete(`${e}|${r[0]},${r[1]}`))}}),N=o,P=0,j=void 0,F=void 0,dt(),Ke(o,Oe)}function Ke(e,t){let n=++Ge;$=!0,I=performance.now(),e.world.update([T.position.x,0,T.position.z],{budgetMs:t,maxCooksPerUpdate:ke,signal:e.abort.signal,...e.tap===void 0?{}:{gpu:e.tap}}).then(t=>{if(e.disposed)return;P=t.pending,F=t.elapsedMs;let n=e.tap?.drain();n!==void 0&&(j=n),dt(),L&&(L=!1,Z(ut()))}).catch(t=>{t instanceof d||(console.error(t),e.disposed||Z(`update failed: ${je(t)}`))}).finally(()=>{n===Ge&&($=!1)})}var z=0,qe=0,B=new Set;window.addEventListener(`keydown`,e=>{let t=e.key.toLowerCase();if(t===`g`){J(b===`gpu`?`cpu`:`gpu`);return}B.add(t)}),window.addEventListener(`keyup`,e=>B.delete(e.key.toLowerCase())),document.addEventListener(`visibilitychange`,()=>{document.visibilityState===`visible`&&(I=performance.now())});var V=me({title:`infinite world`,info:`Unbounded landmark level + world-anchored rock cells streamed around a flying camera. Steer with A/D or arrow keys.`}),Je=document.createElement(`style`);Je.textContent=`
...
`,document.head.appendChild(Je);var Ye=V.el.querySelector(`.pcg-stats`)?.previousElementSibling??V.el;V.addSeed(_,e=>{_=e,R()}),V.addSlider(`speed`,{min:0,max:80,step:1,value:v,format:e=>`${e} u/s`},e=>{v=e}),V.addSlider(`gen radius`,{min:60,max:240,step:10,value:y,format:e=>`${e} u`},e=>{y=e,R()}),V.addSlider(`cell size`,{min:20,max:80,step:20,value:S,format:e=>`${e} u`},e=>{S=e,Pe(),R()}),V.addCheckbox(`world-anchored`,C,e=>{C=e,lt(),R()}),V.addCheckbox(`halo (4 u)`,Te,e=>{Te=e,R()}),V.addCheckbox(`cell grid`,Ee,e=>{Ee=e,D&&(D.visible=e),Fe()});var H=document.createElement(`div`);H.className=`pcg04-seg`;var U=document.createElement(`button`);U.type=`button`,U.textContent=`GPU per-node`;var W=document.createElement(`button`);W.type=`button`,W.textContent=`CPU`,H.append(U,W);var G=document.createElement(`p`);G.className=`pcg04-hint`,G.textContent=`G switches path · A/D or ←/→ steer`;var K=document.createElement(`div`);K.className=`pcg-row`;var Xe=document.createElement(`label`);Xe.textContent=`derived specs`;var q=document.createElement(`input`);q.type=`checkbox`,q.checked=x,q.addEventListener(`change`,()=>{x=q.checked,R()}),K.append(Xe,q),Ye.prepend(H,G,K);function J(e){if(e!==b){if(e===`gpu`&&!M()){Z(A?Q():k??`GPU path not ready yet.`);return}b=e,Y(),R(),Z(ut())}}U.addEventListener(`click`,()=>J(`gpu`)),W.addEventListener(`click`,()=>J(`cpu`));function Y(){U.setAttribute(`aria-pressed`,String(b===`gpu`)),W.setAttribute(`aria-pressed`,String(b===`cpu`)),U.disabled=!M(),U.title=k===void 0?A?Q():``:k,q.disabled=!M()||b===`cpu`}var X=V.addStat(`adapter`),Ze=V.addStat(`fps`),Qe=V.addStat(`rock source`),$e=V.addStat(`rock cells`),et=V.addStat(`cooked / evicted`),tt=V.addStat(`pending`),nt=V.addStat(`instances`),rt=V.addStat(`position`),it=V.addStat(`cook`),at=V.addStat(`resident runs / fused members`),ot=V.addStat(`device dispatches`),st=V.addStat(`gpu fallbacks`),ct=V.addStat(`status`);function Z(e){ct(e)}function lt(){Qe(C?`pointScatterInWorld`:`pointScatterInBounds`)}function Q(){return`device lost (${Ve}) — the GPU path is disabled; reload for a fresh device`}function ut(){return A?Q():b===`cpu`?`CPU path — no resolver passed to the cook`:M()?x?`GPU per-node — combinator fields accepted via acceptDerivedSpecs`:`GPU per-node — acceptDerivedSpecs off, so every field falls back`:k??`requesting WebGPU adapter…`}function dt(){if(it(F===void 0?`–`:`${F.toFixed(1)} ms`),j===void 0){at(N?.tap===void 0?`– (no GPU resolver)`:`–`),ot(`–`),st(`–`);return}at(`${j.residentRuns} / ${j.fusedNodes}`),ot(String(j.dispatches)),st(we(j.fallbacks))}V.addNote(`Drag “cell size”: the blue grid re-cells the world and the rocks do not move or resize. Untick “halo” to watch every border grow a band of undersized rocks, and “world-anchored” to watch the same drag re-roll the world from scratch.`);var ft=V.addCollapsible(`anchoring · what the fine level actually does`);ft.textContent=`pointScatterInWorld scatters over an INFINITE lattice fixed to
...
update cooks nothing.`;function mt(e){A=!0,Ve=e,O=`device lost`,X(O),Z(Q()),Y(),b===`gpu`&&(b=`cpu`,Y(),R())}async function ht(){let e=await xe(mt);if(`error`in e){k=e.error,O=`no WebGPU adapter`,X(O),b=`cpu`,Y(),Z(`${e.error} Running the CPU path.`);return}try{let t=Se(e);ze=t.derived,Be=t.strict}catch(e){k=`GpuFieldEvaluator construction failed: ${je(e)}`,O=`evaluator unavailable`,X(O),b=`cpu`,Y(),Z(`${k} Running the CPU path.`);return}A||(O=e.label,X(O),Y(),Z(ut()),b===`gpu`&&R())}var $=!1,gt=p(e=>Ze(e));X(O),Z(`requesting WebGPU adapter…`),Y(),lt(),dt(),Pe(),R(),ht(),Me(e=>{gt(),qe=(B.has(`a`)||B.has(`arrowleft`)?1:0)-(B.has(`d`)||B.has(`arrowright`)?1:0),z+=qe*1.1*e,T.position.x+=Math.sin(z)*v*e,T.position.z+=Math.cos(z)*v*e,T.position.y=26,T.lookAt(T.position.x+Math.sin(z)*60,5,T.position.z+Math.cos(z)*60),Fe();let t=N;if(t&&!$?Ke(t,De):$&&!L&&performance.now()-I>Ae&&(L=!0,Z(`a cook has been in flight for over 5 s — the GPU device may have been lost.`)),t){let e=t.world.stats(),n=e.levels.find(e=>e.name===`rocks`)?.cellCount??0;$e(String(n)),et(`${e.totalCooked} / ${e.totalEvicted}`),tt(String(P));let r=0;for(let e of t.cellInstances.values())r+=e;nt(String(r)),rt(`${T.position.x.toFixed(0)}, ${T.position.z.toFixed(0)}`)}});