import{Ca as e,Ea as t,Ei as n,Fi as r,Gr as i,Gt as a,Hi as o,Ii as s,Kr as c,Kt as l,Mi as u,Ni as d,Pi as f,Q as p,Ri as m,Sa as h,Si as g,Ui as _,Vi as v,_a as y,ba as b,da as x,et as S,fa as C,ga as w,ha as ee,ji as T,l as E,la as D,ma as O,oa as te,pa as ne,qr as re,sa as ie,u as ae,ua as k,va as oe,xa as se,ya as ce,zi as le}from"./mobile-DnvKbL5k.js";var A=class extends Error{constructor(e){super(e),this.name=`GpuCompileError`}};function j(e,t){let n=Math.fround(e);if(!Number.isFinite(n))throw new A(`${t}: value ${e} is not representable as a finite f32 (WGSL kernels compute in f32; keep magnitudes within ~3.4e38)`);return Object.is(n,-0)?`-0f`:`${String(n)}f`}function ue(e){return`${e>>>0}u`}function M(e){return`0x${(e>>>0).toString(16).padStart(8,`0`)}u`}var N=M,de=j(34028234663852886e22,`internal f32 max`);function fe(e,t){let n=N(e);for(let e of t)n=`pcg_hash_mix(${n}, ${e})`;return`pcg_hash_finalize(${n})`}function pe(){let e=[];for(let t=0;t<12;t++){let n=e=>j(v[t*3+e],`internal GRAD3`);e.push(`  vec3<f32>(${n(0)}, ${n(1)}, ${n(2)}),`)}return`var<private> PCG_GRAD3: array<vec3<f32>, 12> = array<vec3<f32>, 12>(
${e.join(`
`)}
  ...
  return ${P(`internal PERLIN_SCALE`)(le)} * pcg_mix(
  ...
  let t = ${P(`internal simplex R2`)(r)} - x * x - y * y - z * z;
  if (t <= 0f) {
    return 0f;
  }
  ...
  let s = (p.x + p.y + p.z) * ${P(`internal simplex F3`)(d)};
  let i = i32(floor(p.x + s));
  let j = i32(floor(p.y + s));
  let k = i32(floor(p.z + s));
  let t = f32(i + j + k) * ${P(`internal simplex G3`)(f)};
  let x0 = p.x - (f32(i) - t);
  let y0 = p.y - (f32(j) - t);
  let z0 = p.z - (f32(k) - t);
  var i1: i32;
  ...
  let x1 = x0 - f32(i1) + ${P(`internal simplex G3`)(f)};
  let y1 = y0 - f32(j1) + ${P(`internal simplex G3`)(f)};
  let z1 = z0 - f32(k1) + ${P(`internal simplex G3`)(f)};
  let x2 = x0 - f32(i2) + ${P(`internal simplex 2*G3`)(2*f)};
  let y2 = y0 - f32(j2) + ${P(`internal simplex 2*G3`)(2*f)};
  let z2 = z0 - f32(k2) + ${P(`internal simplex 2*G3`)(2*f)};
  let x3 = x0 - 1f + ${P(`internal simplex 3*G3`)(3*f)};
  let y3 = y0 - 1f + ${P(`internal simplex 3*G3`)(3*f)};
  let z3 = z0 - 1f + ${P(`internal simplex 3*G3`)(3*f)};
...
`))}var z={euler:`fn pcg_quat_from_euler_deg(r: vec3<f32>) -> vec4<f32> {
...
${_.join(`
`)}`,...he(c.libRoots),...c.helperBlocks(),`@compute @workgroup_size(${Ae})
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  let i = gid.x + params.chunkOffset;
  if (i >= params.count) {
    return;
