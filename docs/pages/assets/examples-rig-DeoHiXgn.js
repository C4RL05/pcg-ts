var e=`{\r
  ...
  "params": [\r
    ...
    {\r
      ...
      "min": 0.15,\r
      "max": 1.2,\r
      "description": "Half the width of the box truss, in world units: the distance from the spine out to each chord. The four chords sit at ± this along the curve normal and binormal, and the diagonal braces and component mounts at this × √2 — twenty readings of one number, previously written in four different float spellings of it, so this is the knob that sizes the truss. Eighteen of the twenty are in THIS graph; the other two are inside the cable \`forEach\` body and reach it through the wrapper's own \`halfWidth\` param, because a body is bound by its wrapper and by nothing else. That boundary is why \`pcg validate --params\` counts NINETEEN and not twenty, and both numbers are right: \`--params\` describes the graph it was handed, where the body's two readings appear as the single \`wrapWraps.halfWidth\` slot that feeds them — eighteen plus one — and a body is a different graph, which the outer count cannot see into. The two were missed when the other eighteen were collapsed, and the cables sat inside the truss at any value but the default until they were plumbed through."\r
    },\r
    {\r
      "name": "braceRadius",\r
      "value": 0.03,\r
