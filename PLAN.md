...
### ~~Two corpus graphs still build on the curve's ruler, and one of them contradicts the node it demonstrates~~ -- TAKEN 2026-09-05, both swapped; `sampleStepAttr` still open

**BOTH GRAPH EDITS SHIPPED** in `5512b4c`, one param each plus the prose. Neither
picture moves: the scatter cook differs ONLY in the primitive `length` column
and by one f32 ulp, transforms byte-identical and 35 markers either way; the
runs cook keeps `P` byte-identical and moves `color` by 1.19e-7, one f32 ulp at
0.5, because `since / (lapLength/4)` cancels the ruler. `tests/graphs.golden.json`
and `docs/gallery.html` both regenerated byte-identical, which is what confirms it.

**THE PREDICTION BELOW ABOUT THE SCATTER GRAPH WAS RIGHT FOR THE WRONG REASON,
and the correction is the useful part of this entry.** The count does still round
to 35 and 100.4906 is still true -- but not because the ruler gap is small on a
gentle curve. That loop is a 64-segment CIRCLE sampled at 128 = 2x64, so every
other sample lands exactly ON an input vertex and no chord spans a corner at all.
The gap is 0.0057 ppm, and the 0.0759 ppm the column reads is two sums rounding
to f32. Change nothing but `count` to 127 and the SAME curve parts by 405 ppm; a
16-gon at 128 reads exactly 0.00. So the gap is not predictable from the shape,
and "coarser is worse" is false. I wrote the shape explanation into the graph's
prose first and verification caught it.

**THE 0.87126/0.87233 FIGURES BELOW ARE `basics-fit-runs`' LAP, NOT THIS ONE.**
`basics-runs-along-a-path` measures 0.5521 to 0.5563, a 0.75% spread, against a
ruler error of 381 ppm -- so the constant `seg` is about TWENTY TIMES the error
the swap removed, which is why the shipped prose claims exactness about the TOTAL
and never about a step. And that 381 is not "a 40-gon" as a mechanism either: a
40-gon CIRCLE at 240 reads 0.00, and the 381 comes from that ring being an
ELLIPSE whose 40 segments are unequal, so no count lands them all on vertices.

**WHAT THE SWAP ACTUALLY BOUGHT**, stated properly: the error changed KIND. Every
run-end error was positive before (+40.9, +735.7, +40.9, +735.7 ppm) because the
shortfall is a one-sided bias; after, they straddle zero and the four runs close
on the lap to 3e-7 instead of 0.0509 past it. Two of the four are individually
FURTHER from their true arc than before. A cancelled bias is not every run
improving, and the shipped prose says so.

**`sampleStepAttr` IS STILL OPEN** and is the only part of this entry not taken.
Being exact per step still needs each sample's own chord, which no report writes.
