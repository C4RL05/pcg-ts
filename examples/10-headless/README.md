...
```
npm run build
node examples/10-headless/run.mjs          # cook it, print stats and bounds
node examples/10-headless/run.mjs 12345    # same graph, another seed
```
...
pcg inspect  examples/10-headless/graph.json --node height --rows 3
...
```
node bin/pcg.mjs cook examples/10-headless/graph.json --stats
```
