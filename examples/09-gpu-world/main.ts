...
function onDeviceLost(detail: string): void {
  ...
  setStatus(deviceLostStatus());
  console.error(`09-gpu-world: WebGPU device lost — ${detail}`);
  if (rig) teardown(rig);
  rig = undefined;
  paintMode();
}
...
async function boot(): Promise<void> {
  ...
  if (device !== undefined) {
    try {
      ...
      console.warn("09-gpu-world: shared-device renderer failed:", err);
    }
  }
  if (gl === undefined) {
  ...
  if (device !== undefined && residentBlocker === undefined) {
    try {
      ...
      console.warn("09-gpu-world: device instance adapter unavailable:", err);
    }
  }
  ...
  if (!residentAvailable()) {
    mode = "cpu";
    console.info(`09-gpu-world: device path unavailable — ${residentBlocker ?? "unknown reason"}`);
  } else {
    console.info(
      `09-gpu-world: WebGPU ready — ${adapterLabel}; ` +
...
void boot().catch((err: unknown) => {
  ...
  console.error("09-gpu-world: boot failed:", err);
});
