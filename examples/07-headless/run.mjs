 ...
 *   node examples/07-headless/run.mjs [seed]
 *
 ...
 *   pcg inspect  examples/07-headless/graph.json --node height
...
try {
  ...
  if (err?.code === "ERR_MODULE_NOT_FOUND") {
    console.error(
      "07-headless: dist/index.js not found — run `npm run build` first, then re-run this script.",
    );
    process.exitCode = 1;
  } else {
    throw err;
