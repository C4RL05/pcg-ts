/**
 * The example corpus: enumerating `graphs/{basics,pipeline}-*.json`
 * and rendering the generated index (`docs/graphs.md` + `docs/graphs.json`).
 *
 ...
 *
 * {@link loadGraphs} is here rather than in either caller for a second
 * reason: the corpus CI (`tests/graphs.test.ts`) enumerates the same
 ...
 * dropped into `graphs/` is picked up by the index, the golden
 ...
 */
export const GRAPH_PREFIXES: readonly string[] = ["basics-", "examples-", "pipeline-"];

/** One rendered index: the bytes of each generated file. */
export interface ExampleIndex {
  /** Contents of `docs/graphs.json`. */
  readonly json: string;
  /** Contents of `docs/graphs.md`. */
...
export interface GraphFile {
  ...
  /** Repository-relative path, e.g. `graphs/basics-scatter-in-bounds.json`. */
...
/**
 * Read every corpus graph under `<root>/graphs`, sorted by file
 ...
 */
export function loadGraphs(root: string): GraphFile[] {
  const dir = join(root, GRAPHS_DIR);
  const names = readdirSync(dir)
    .filter((name) => GRAPH_PREFIXES.some((p) => name.startsWith(p)) && name.endsWith(".json"))
    .sort();
  return names.map((file) => {
    ...
    try {
      ...
      throw new Error(
        `${GRAPHS_DIR}/${file}: not valid JSON (${err instanceof Error ? err.message : String(err)})`,
      );
    }
    return { file, path: `${GRAPHS_DIR}/${file}`, absolutePath, json };
  });
}
...
function fail(file: string, message: string): never {
  throw new Error(`${GRAPHS_DIR}/${file}: ${message}`);
}
 ...
 */
export function describeExample(entry: GraphFile): ExampleEntry {
...
// ---------------------------------------------------------------------------
// docs/graphs.md — one section per example.
// ---------------------------------------------------------------------------
 ...
 * entries are sorted by file name (in {@link loadGraphs}, so directory
