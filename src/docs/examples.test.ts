/**
 * `docs/examples.md` and `docs/examples.json` are generated from the
 * corpus under `examples/graphs/`, and nothing in the build regenerates
 ...
 * generator uses (src/docs/examples.ts, imported here from source so
...
import { describeExample, loadCorpus, renderExampleIndex } from "./examples.js";
...
describe("example index docs", () => {
  ...
  it("docs/examples.json matches the corpus on disk", () => {
    const committed = readDoc("examples.json");
    if (committed !== json) throw new Error(drift("examples.json", committed, json));
  });

  it("docs/examples.md matches the corpus on disk", () => {
    const committed = readDoc("examples.md");
    if (committed !== markdown) throw new Error(drift("examples.md", committed, markdown));
  });

  it("renders identically twice (deterministic output)", () => {
    const again = renderExampleIndex(loadCorpus(ROOT).map(describeExample));
