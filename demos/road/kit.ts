/**
 * Reading a measured kit, and putting its placements on THIS spline.
 *
 * WHAT THIS IS FOR, and it is the check everything else rests on. The kit
 * describes a real circuit's environment art in track-relative
 * coordinates — arc length, a signed lateral offset, a height, all in
 * half-widths. If those coordinates mean the same thing on both sides,
 * dropping 442 of them onto a spline they were never measured from
 * produces something that reads as a track. If either side has a
 * convention backwards it produces a cloud, and no amount of agreement
 * about occupancy figures would have caught it.
 *
 ...
 *
 * THE FILE IS NOT IN THIS REPOSITORY and never will be: it is derived
 * measurement of a copyrighted game, it lives outside both repos, and it
 * is not ours to redistribute. The demo fetches `/kit.json` if someone
 * has put one there — gitignored — and does without when nobody has.
 */
...
export interface KitBox {
  ...
  /** panel / leg / post / span / head / mass — inferred, not measured. */
   ...
   *
   * NEAR ZERO MEANS A SHEET — not a thin wall, and not a wall the export's
   * grid could not resolve. Three quarters of this kit is single-sided
   * surface, which is how the original was modelled. For those, depth is
   * a number this side invents and owns; there is nothing to recover.
   */
  readonly thickness?: number;
}

/** One instance of an asset, placed on the circuit it was measured from. */
...
export interface Kit {
  readonly track: { readonly lapLengthW: number; readonly halfWidthGameUnits: number };
 ...
 *
 * THE STATION IS SCALED, not wrapped by the caller. A kit measured on a
 * 403 W circuit dropped onto a lap of a different length has to decide
 * what "station 200" means: the same DISTANCE (so a shorter lap runs out
...
export function placeKit(
  ...
  for (const pl of kit.placements) {
    ...
    for (const b of pl.boxes) {
      ...
      // not a world-space offset, and not the game's world axes. This is
